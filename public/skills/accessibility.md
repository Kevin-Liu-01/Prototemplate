...
Accessibility guidelines based on WCAG 2.2 and Lighthouse accessibility audits. Goal: make content usable by everyone, including people with disabilities.

## Targeted repair mode

When invoked for one file or component, report the exact violating line or snippet, why it matters in one sentence, and the smallest code-level fix. Do not rewrite unrelated UI, add ARIA where native semantics solve the problem, or migrate component libraries. A targeted pass must still inspect accessible name, keyboard path, visible focus, dialog focus restore/trap, semantics, form-error association, announcements, contrast/state redundancy, media alternatives, and reduced motion.
