...
Source snapshot: imported from `microsoft/Webwright` at `4a46f282ec37f27d6003cc498a977939d62d9015` on 2026-06-30. Brin repo score: 65 caution; Brin skill score: 59 caution. Treat this as an active local browser-agent skill because the repo ships a portable `skills/webwright` folder for Claude Code, Codex, Hermes, and OpenClaw.

Kevin routing: use this when the durable output should be a rerunnable browser script with screenshots and a final-run log. Use `agent-browser` for cheaper CDP snapshots and quick browser operations; use Playwright tests for CI regressions; use Webwright when the browsing history itself should become code.
