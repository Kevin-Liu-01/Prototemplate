...
5. **Component and interaction sources** → read `wiki/design/component-library-sources.md` and use `animated-component-libraries` when a shadcn registry, Skiper UI, Cult UI, Magic UI, React Bits, or a modern CSS technique can save time without compromising taste
6. **Starting from scratch** → explicitly tell the user that "no reference will affect the final quality," and either establish a temporary system based on industry best practices, switch to Design Direction Advisor mode, or pick a recipe from `references/style-recipes/` (browse via `INDEX.md`) and confirm with the user
...
Matching the existing visual vocabulary is the prerequisite for native-feeling integration; newly added elements should be **indistinguishable from the originals**.
...
- Component sources: [local system / shadcn / Skiper UI / Cult UI / Magic UI / React Bits / custom]
```
