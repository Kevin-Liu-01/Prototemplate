...
## Kevin-Wiki activation boundary

The upstream `/media-use` helper is retained under `skills/in-progress/media-use`, not active runtime. Do not try to load it or invoke its provider, credential, model-download, URL-fetch, or telemetry paths. For media, use user-supplied project assets or an already-approved narrow local media skill; ask for explicit authorization before any provider call, credential use, remote download, or account mutation. This boundary does not block ordinary HyperFrames HTML/CSS/SVG authoring, snapshots, validation, deterministic rendering, or visual review.
...
| Images, icons, logos, audio, captions, grades, LUTs, reusable media                                                 | Approved local media route; upstream `/media-use` is retained in progress pending activation review |
...
Broad feedback about how photographic media looks or behaves routes to an
approved local media skill, even when the user never says “color grading” or “effect”: fix
dark/flat/boring footage, stylize a clip, hide a face, or improve a media
reveal. Preserve provenance, consent, and rights; do not substitute a generic
LUT, CSS filter/overlay, or opacity tween for an established project primitive.
Keep text/layout/motion-only edits in their owning domain.
