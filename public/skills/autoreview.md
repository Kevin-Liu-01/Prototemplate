...
- user asks to review a branch, PR, or diff "since X" against an issue, PRD, spec, or documented standard

## Standards / Spec Axis

For branch, PR, or "review since X" work, preserve two separate review axes from Matt Pocock's `review` skill:

- **Standards**: Does the diff follow the repo's documented coding, product, UI, security, and process standards?
- **Spec**: Does the diff implement the originating issue, PRD, user request, or acceptance criteria?

Pin the fixed point before reviewing. Prefer the true PR base; otherwise use the user-supplied commit, branch, tag, or merge-base. Confirm the diff is non-empty before spending review time.

Find the spec source in this order: issue/PR references in commit messages, a path the user gave, PRD/spec files under `docs/`, `specs/`, `.scratch/`, or the original user request in the current thread. If no spec exists, say "no spec available" and run the Standards axis only.

Find standards sources from `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.cursor/rules/`, `CONTRIBUTING.md`, `CODING_STANDARDS.md`, `docs/`, and any repo-local design or security guides.

Add the Fowler smell baseline to the Standards axis even when the repo documents no standards. A repo standard overrides this baseline, and every smell is a judgement call, not a hard violation. Skip anything tooling already enforces. Use labels like "possible Feature Envy" and tie the finding to the diff hunk:

| Smell | What to look for | Typical fix |
| --- | --- | --- |
| Mysterious Name | Names that do not reveal what a value, function, or type does or holds. | Rename it; if no honest name exists, clarify the design. |
| Duplicated Code | The same logic shape appears in multiple hunks or files. | Extract the shared shape or choose one owner. |
| Feature Envy | A method reaches into another object's data more than its own. | Move behavior toward the data it uses. |
| Data Clumps | The same fields or params travel together repeatedly. | Introduce a named type/value object. |
| Primitive Obsession | Strings/numbers/booleans stand in for domain concepts. | Give the concept a small type or enum. |
| Repeated Switches | The same `switch`/`if` cascade recurs on the same concept. | Centralize dispatch, use a map, or use polymorphism. |
| Shotgun Surgery | One logical change forced scattered edits across many files. | Gather the change boundary into one module. |
| Divergent Change | One module changes for several unrelated reasons. | Split responsibilities. |
| Speculative Generality | Abstractions, params, hooks, or extension points serve needs the spec does not have. | Delete or inline until a real second use exists. |
| Message Chains | Callers walk through long object chains such as `a.b().c().d()`. | Hide the walk behind one method at the right boundary. |
| Middle Man | A function/class mostly delegates without adding policy. | Call the real target directly or give the wrapper real ownership. |
| Refused Bequest | A subtype/implementer ignores most inherited behavior. | Prefer composition or a smaller interface. |

When reporting findings, keep the axes separate. Do not let a clean Standards result hide a Spec miss, and do not let correct functionality hide a standards violation. If using the structured helper, pass the spec and standards context through a prompt file or dataset when practical; otherwise apply the split manually after reading the diff.
...
## Deterministic Coverage Lane

For a large or heterogeneous diff, substantial untracked work, or an explicit
full-coverage request, add OpenCodeReview's delegation mode when a reviewed
`ocr` CLI is already available. It supplies deterministic file selection and
path-rule resolution; this skill remains the review and judgment owner.

1. Freeze the same base/ref and authority used by the Standards and Spec axes.
2. Run `ocr delegate preview --format json` with the matching workspace,
   commit, or range flags. Do not install `ocr` merely because a review began.
3. Create a ledger from every `reviewable_files` row, keyed by `(path, status)`.
   Preserve excluded rows and their reasons. The status is part of identity
   because one workspace can contain a staged deletion and an untracked
   recreation at the same path.
4. Run `ocr delegate rule --format json <paths...>` in bounded batches, inspect
   every file against its resolved rules plus this skill's Standards and Spec
   contracts, and mark each ledger row `reviewed` or `skipped` with a concrete
   reason.
5. Report totals, reviewed rows, skipped rows, and coverage rate. Never return
   a clean result while a previewed row is unaccounted for.

Delegation mode must not silently widen authority. Provider configuration,
telemetry, comment publication, fixes, and global CLI installation are separate
decisions. If `ocr` is unavailable, keep the same ledger invariant using Git
diff/status evidence rather than degrading to undocumented sampling.
