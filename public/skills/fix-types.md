...
<!-- auto-promoted from skills/engineering/dedalus-fix-types by sync-dedalus on 2026-06-13; review & generalize any Dedalus-specific references. -->
...
| --- | --- | --- |
| `dedalus-fix-types` | [`references/skills/dedalus-fix-types/SKILL.md`](references/skills/dedalus-fix-types/SKILL.md) | Audit code for loose types that allow semantically invalid values to compile. Introduces newtypes, checked casts, enums over booleans, and documents wire/DB discriminant contracts. Use when writing structs with bare primitives, reviewing database or protocol code, or when a function takes 2+ parameters of the same type. |
