...
Kevin-Wiki admission: installed from `oso95/scroll-world` at exact commit
`71cc36d3bb150248ae36a2c552f9cbf88802a79c`. The MIT repository contains one
skill and a portable scrub engine, but no tests or CI. Treat the generated
experience as pre-rendered video controlled by scroll—not runtime Three.js—and
do not substitute it when a product needs interactive 3D state, geometry,
lighting, or camera control.

The command-line prerequisites are installed locally at `monid@0.1.6` and
`higgsfield@1.1.23`; ffmpeg, ffprobe, Pillow, and Codex are also present. That
does not authorize authentication, generation spend, provider egress, asset
reuse, or publication. Recheck provider/model schemas and current price before
every run, show the estimated budget, and obtain the user's explicit approval
before the first paid generation. Preserve prompt, model, provider, price,
input-asset rights, job IDs, output hashes, and seam/mobile proof in the run
receipt.

Do not accept the upstream engine or its prose QA as executed proof. A target
build still needs a frozen route and viewport, before/after-seam captures,
composition plus velocity review, desktop/mobile network evidence, real or
emulated portrait testing, reduced-motion verification, keyboard/readability
checks, frame and transfer measurements, teardown/cleanup review for framework
mounts, and independent acceptance. A missing connector may degrade only after
the user sees the changed journey; it must not silently convert the promised
continuous flight into a direct crossfade.
