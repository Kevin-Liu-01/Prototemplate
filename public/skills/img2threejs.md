...
Kevin-Wiki admission: this is the complete upstream snapshot at signed commit
`d6673386f89673a58736f8d398dd16ece67874f5`, installed as one cross-agent skill.
The commit is tagged `v1.5-beta`, while upstream's own skill frontmatter still
says `1.4.4`; preserve both identities until upstream reconciles them. The core
suite passed 673 tests locally with 25 environment/showcase skips on 2026-08-11.
The source archive and admission proof are content-addressed under
`.brain/artifacts/x/2079267787298746709/img2threejs/`.

Admission boundary: the core `forge/` workflow is local, Python-standard-library
code. Do not treat the complete snapshot as blanket authority to run its optional
network paths. `integrations/vision/reference_vision.py` downloads model files,
`forge/stage1_intake/fetch_cs2_metadata.py` can fetch metadata and images,
`scripts/issue_triage.py` calls GitHub and may use a token, and the Playwright
capture adapter requires a separate package/browser install. Invoke any of those
only when the task needs that exact capability, after declaring destination,
credentials, downloads, and external effects; issue triage remains maintainer
tooling rather than a normal reconstruction step.
