...
10. **AI SDK 7 is broad and moving** - Verify current docs/source for `WorkflowAgent`, `Agent`, `HarnessAgent`, tool approvals, tool/runtime context, files, skills, MCP Apps, sandbox support, terminal UI, telemetry, lifecycle events, and reasoning controls before choosing a primitive.
...
Do not assume one agent primitive is always correct. Search `node_modules/ai/docs/` and source for current agent APIs, then choose the smallest primitive that matches the work:

- `WorkflowAgent` / workflow integration for durable or resumable agents that must survive restarts, deploys, interruptions, or delayed approvals.
- `HarnessAgent` / harness adapters when embedding Claude Code, Codex, Pi, or another mature harness behind AI SDK-compatible streams.
- `Agent`, `generateText`, or `streamText` when the loop is simple and the app owns the tool/control flow directly.
...
- Wiki: `wiki/tools/vercel-ai-sdk.md` for current routing, and `wiki/tools/ai-test-kit.md` for deterministic AI SDK test helpers.
