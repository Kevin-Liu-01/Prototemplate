...
Before applying this table, check current package authority:

```bash
npm view next version dist-tags --json
```

Use the newest patch available on the target release line, not merely the first fixed version in an advisory. The 2026-05-07 security release fixed multiple high/moderate/low issues in `next@16.2.6` and `next@15.5.18`; as of 2026-07-03, `next@latest` is `16.2.10`, `next@preview` is `16.3.0-preview.5`, and `next@canary` is `16.3.0-canary.76`.
...
| 15.3.x | `next@15.3.6` |
| 15.4.x | `next@15.4.8` |
| 15.5.x | `next@15.5.18` for the May 2026 security train; prefer latest patch if newer |
| 16.0.x | `next@16.0.11` |
| 16.1.x | `next@16.1.5` |
| 16.2.x | `next@16.2.6` for the May 2026 security train; prefer latest patch if newer |
...
| --- | --- | --- |
| `next-best-practices` | [`references/skills/next-best-practices/SKILL.md`](references/skills/next-best-practices/SKILL.md) | Next.js best practices - file conventions, RSC boundaries, data patterns, async APIs, metadata, error handling, route handlers, image/font optimization, bundling |
| `next-cache-components` | [`references/skills/next-cache-components/SKILL.md`](references/skills/next-cache-components/SKILL.md) | Next.js 16 Cache Components - PPR, use cache directive, cacheLife, cacheTag, updateTag |
| `next-forge` | [`references/skills/next-forge/SKILL.md`](references/skills/next-forge/SKILL.md) | next-forge expert guidance — production-grade Turborepo monorepo SaaS starter by Vercel. Use when working in a next-forge project, scaffolding with `npx next-forge init`, or editing @repo/* workspace packages. |
| `next-upgrade` | [`references/skills/next-upgrade/SKILL.md`](references/skills/next-upgrade/SKILL.md) | Upgrade Next.js to the latest version following official migration guides and codemods |
