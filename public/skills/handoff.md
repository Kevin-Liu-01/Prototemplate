...
## When handoff is the correct transition

Use this route when current context is valuable and the recipient, harness,
directory, project, or human changes. Otherwise choose the cheaper transition:

- continue when the next step is bounded and the current session is still useful;
- start fresh when the old exploration is disposable, after durable facts are written;
- delegate a bounded AFK branch when the parent should retain synthesis and proof;
- compact when the same owner continues and most context remains relevant.

Handoff preserves momentum, not canonical truth. Reference existing specs,
ADRs, owner pages, commits, diffs, tests, and proof instead of copying them into
the prompt. [Source: X/@mattpocockuk `2079879414297330146` visual decision tree,
2026-07-22]
