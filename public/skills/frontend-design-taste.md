...
- Generic or mixed icon imports with no recorded family/style decision. Preserve
  a coherent incumbent, or admit a challenger through the product coverage,
  license, wrapper, accessibility, and bundle gate below.

## Icon-System Gate

Before implementing or recommending an icon-library change, read
`~/Documents/GitHub/kevin-wiki/wiki/design/ui-design-defaults.md#icon-system-admission-not-pack-swapping`
and the project's existing icon imports.

1. Name the incumbent failure. "Lucide is common" is not enough; use missing
   concepts, wrong optical voice, weak state grammar, framework mismatch,
   license, or measured bundle cost.
2. Render 20–30 actual product concepts at 16, 20, and 24px, including
   direction, destructive action, status, empty/error, and domain nouns.
3. Select one family and one style. Record exact package/source revision,
   license, grid, size/stroke/fill tokens, missing concepts, and the rollback.
4. Centralize direct named imports behind an `AppIcon` wrapper and semantic map.
   No wildcard imports, runtime package-wide lookup, or cross-pack mixing without
   measured output proof.
5. Verify decorative/meaningful semantics, accessible names, RTL, high contrast,
   and every interaction state at the smallest shipping size.
6. Keep the incumbent and record a no-change receipt when the challenger does
   not materially improve the product.

Use [[thesvg]] for brand/company marks. That is a different job from interface
glyphs.
