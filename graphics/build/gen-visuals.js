...
add('C5-one-tree', 'C', 'One tree', 'silk3', 'The sidebar drawn as the tree it is, mapped onto the real crop.', () => {
  ...
  groups.forEach(([k, n, r], i) => { const gy = 200 + i * 130; const gx = 420; h += elbow(rootX - 60 + 132 - 1, rootY, gx - 4, gy + 12, { mx: 320, color: 'rgba(96,165,250,.7)', end: true }); /* the badge is 132 wide from rootX - 60; the branch starts on its edge */ h += label(gx, gy - 11, `${k}&nbsp;<span style="font:500 26px 'Geist Mono';color:var(--muted);letter-spacing:.02em">${n} pages</span>`); for (let j = 0; j < n; j++) h += `<div class="wire bar" style="left:${px(gx + 380 + (j % 6) * 24)};top:${px(gy + 2 + Math.floor(j / 6) * 24)};width:16px;height:16px;border-radius:3px;background:rgba(96,165,250,.95);box-shadow:0 0 0 2px rgba(9,9,11,.7)"></div>`; h += hl(o, s, r, { pad: 3, style: 'box-shadow:none;border-width:2.5px' }); h += elbow(gx + 530, gy + 12, X + r.x * s - 6, Y + (r.y + r.h / 2) * s, { mx: 1100, color: 'rgba(96,165,250,.5)', end: true }); });
  return h;
});
...
add('E2-solid-vs-outline', 'E', 'Two tiers of icons', 'silk1', 'The new docs’ icon rule, from real captures: controls (search, language, copy page, and the half-filled circle of the theme toggle) keep thin glyphs; icons that carry meaning (the Core, CLI and Integrations cards, the Libraries feature, the Changelog and Contact links) are solid. Below, the old page did the reverse: a solid theme toggle over outline tiles.', () => {
  ...
  const tile = (x, y, file, lab, muted = false) => `<div class="txt" style="left:${px(x)};top:${px(y)};width:170px;text-align:center"><div style="display:grid;place-items:center;margin:0 auto;width:100px;height:100px;border-radius:8px;border:1px solid var(--border2);background:#0c0c0f;color:${muted ? '#a1a1aa' : '#fafafa'}">${file.startsWith('glyph:') ? `<span style="font-size:50px;line-height:1">${file.slice(6)}</span>` : svgFile(file, 48)}</div><div style="margin-top:10px;font:500 26px/1.2 'Inter';letter-spacing:-.01em;color:${muted ? 'var(--muted)' : 'var(--text)'}">${lab}</div></div>`;
  h += tag('Controls · outline', 'cursor-arrow-rays', 64, 110, 'dark');
  // the theme toggle is a text glyph in the docs (◐ light, ◑ dark), not an icon; drawn as the dark page shows it
  [['10-outline-open-search.svg', 'Search'], ['06-outline-english-us.svg', 'Language'], ['12-outline-copy-page.svg', 'Copy page'], ['glyph:◑', 'Theme']].forEach(([f, lab], i) => { h += tile(64 + i * 170, 190, f.startsWith('glyph:') ? f : NEW + f, lab); });
...
add('F5-poster', 'F', 'Poster', 'silk5', 'The hit list regrouped into clutter, navigation and language, every item marked with the set’s red x; the group names are the set’s ordinary labels.', () => {
  ...
  groups.forEach(([name, items], c) => { const x = X0 + c * (CW + G); h += tag(name, null, x, 96, 'dark'); let y = 190;
    items.forEach((k) => { h += `<div class="txt" style="left:${px(x)};top:${px(y)};width:${px(CW)};display:flex;align-items:flex-start;gap:12px;white-space:normal;font:600 34px/1.2 'Inter';letter-spacing:-.02em;color:rgba(250,250,250,.94)"><span style="display:inline-grid;place-items:center;flex:none;width:36px;height:36px;margin-top:2px;border-radius:50%;background:rgba(240,82,79,.94);color:#fafafa;box-shadow:0 0 0 3px rgba(9,9,11,.6)">${ico('x-mark', 20)}</span><span>${k}</span></div>`; y += Math.ceil(k.length / CHARS) * LINE + 22; }); });
  return h;
}, { wash: .25 });
