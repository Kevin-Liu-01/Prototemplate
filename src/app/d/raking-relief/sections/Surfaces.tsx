...
import { CLI_RUN, LOCADEX_DIFF, LOCADEX_FINDINGS, REVIEW_ROWS, STATS } from './content';

/**
 * The product surfaces as four raised tablets laid in a running bond: the
 * libraries and the CLI on the first course, the dashboard and the agent
 * on the second, offset like bricks. Above them, four figures in a ruled
 * register. Every tablet holds a real object: the six packages, one CLI
 * run, the four review rows, the agent's findings.
 */
export default function Surfaces() {
  return (
    <section className='rr-course rr-surfaces' id='platform'>
      ...
      <ul className='rr-stats'>
        {STATS.map((stat) => (
          <li key={stat.label}>
            <span className='rr-stat-value'>{stat.value}</span>
