...
export default function DarkBand() {
  ...
  // Fork material: the CLI cell's bloom and the band's floor are drawn by
  // this page's own engine on permanently dark plates. Their ink is the
  // marquee's bulb colour (C1.5): themeInk reads it from the canvas's
  // computed `color`, which styles.css sets to var(--deco-ornament), so the
  // literal below is only the first frame's ink before the token resolves.
  // applyStyles is off for both: the engine's default inline `width/height:
  // 100%` would override the fixed CSS boxes that fence the texture away
  // from type.
