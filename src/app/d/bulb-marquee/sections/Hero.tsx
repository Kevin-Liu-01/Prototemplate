...
import BoardBar from './deco/BoardBar';
import HouseSign from './deco/HouseSign';
...
/**
 * THE MARQUEE HERO, on the founder stack. Two ranks of surfaces on the
 * hairline mat, separated by 1px seams: a split row (the copy card beside
 * the board) and the trust card below, full width. On mobile the board
 * stacks under the copy.
 *
 * The copy card opens on the crown, the house sign (sections/deco/HouseSign):
 * mark and name between two bulb rows. Then the two authored headline lines
 * with the morphing `language` hinge, the sub with the wordmark inline, the
 * acts, and the real copyable command.
 *
 * The board is the transmission plate as a letter board: a bar on top with
 * the live mark (sections/deco/BoardBar), bulb rows on all four edges, and
 * the Bayer TRANSMISSION filling everything inside them. The pinned source
 * greeting docks on the left edge, wavefronts sweep the width, and real
 * greetings with their locale tags materialise out of the ordered dither as
 * each front reaches them: the show now playing, printed by the field. One
 * hairline wire passes under the field through the dock.
 *
 * The field is 1-bit and theme-following: its ink is the canvas's computed
 * `color` (var(--tc-ink)), re-sampled when data-theme flips, so the board
 * prints the page's warm white on its warm black in both themes. Under
 * prefers-reduced-motion the engine renders one composed still and every
 * bulb row rests fully lit.
 */
    ...
    <section className='tc-sec tch-hero-sec' id='top' ref={root}>
      ...
      <div className='tch-split'>
        <div className='tc-hero tch-card tch-copy'>
          {/* The crown (C1.4): the house sign above the headline. */}
          <HouseSign />
