...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    title: post
      ? `${post.title} — Blog — Signal — GT Redesign`
      : 'Blog — Signal — GT Redesign',
 ...
 * to. The shell is the same one every Signal company page runs; the
 ...
 */
export default async function SingularitySignalBlogPostPage({ params }: Params) {
  const { slug } = await params;
  const post = findPost(slug);
  if (!post) notFound();
