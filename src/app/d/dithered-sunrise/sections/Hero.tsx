...
import { heroSunrise, useDitherField } from '../fields';
import SunCrown from './deco/SunCrown';
...
/**
 * THE SUNRISE HERO, on the founder stack, recomposed so the field FILLS
 * its component (the founder's directive, verbatim).
 *
 * Two ranks of surfaces on the hairline mat, separated by 1px seams: a
 * SPLIT row, the copy card (the SunCrown above the h1, two authored headline
 * lines with the morphing `language` hinge, the sub with the wordmark
 * inline, acts, and the real copyable command) beside the SUNRISE PLATE,
 * the hero's media panel, then the trust card below, full width. On mobile
 * the plate stacks under the copy.
 *
 * The plate is the heroSunrise field (../fields.ts), filled edge to edge: a
 * half sun on a horizon at 72% of the plate, a fan of rays stepped into flat
 * bands before the Bayer screen sees them, dawn arcs drifting outward, and
 * the reflection below as quantized dashes. One hairline wire is the
 * horizon, under the field, on the shell's rule grammar.
 *
 * The field is 1-bit and theme-following: its ink is the canvas's computed
 * `color` (var(--deco-ornament)), re-sampled when data-theme flips, so the
 * dark theme prints the lifted gold and never a stale frame.
 */
...
export default function Hero() {
  ...
  /* The plate composition: the half sun on its horizon, the fan of rays,
     the dawn arcs and the reflection, filling the component. The ink is the
     canvas's computed color (the ornament gold, re-read on a theme flip);
     the literal below is the engine default the hook overwrites at mount.
     reducedMotionTime 5 is a composed still with two arcs mid-fan. */
  const canvasRef = useDitherField(heroSunrise, {
    scale: 3,
    ink: '#070707',
    paper: 'transparent',
    fps: 20,
    ...
    () => {
      ...
      if (em && word) {
        ...
        if (compactEvery) {
          ...
          /* The first dissolve waits out the first-fold capture window, so a
  ...
  return (
    <section className='tc-sec tch-hero-sec' id='top' ref={root}>
      {/* The founder's stack, split: the copy card beside the sunrise plate,
          two cards on the hairline mat with one 1px seam between them; the
          trust row repeats the card grammar below. */}
      <div className='tch-split'>
        <div className='tc-hero tch-card tch-copy'>
          {/* C1.4 the crown: the GT mark seated in a dithered sun, above the h1. */}
          <div data-hero-in>
            <SunCrown />
          </div>
          ...
          {/* The wordmark inline in prose: the monogram at cap height beside the
              name, in the sub's own ink (the production GtLogoText grammar). */}
          <p className='tc-hero-sub' data-hero-in>
            <span className='ds-wordmark'>
              <span className='ds-wordmark-glyph'>
                <GtMark width={19} height={12} />
              </span>
              General Translation
            </span>{' '}
          ...
          {/* The acts carry the dev-first device too: the real, copyable
              command sits with the buttons, on the card and outside the
        ...
        {/* The plate: the hero's media panel, and the sunrise fills all of it.
            The one wire is the horizon, a hairline under the field at the
            sun's waterline. */}
      ...
      {/* The trust card: six real wordmarks in the page's ink, ruled into six
          cells. The claim is the lead; the restraint is the diagram. */}
