...
import { dawnArcs, sunburstFloor, useDitherField } from '../fields';
...
export default function DarkBand() {
  ...
  // C1.5: the CLI cell's dawn arcs and the band's sunburst floor are drawn
  // by this page's own engine, gold Bayer fields on permanently dark plates.
  // The ink is each canvas's computed color (the ornament gold, re-read on
  // a theme flip); the literals below are the engine defaults the hook
  // overwrites at mount. applyStyles is off for both: the engine's default
  // inline `width/height: 100%` would override the fixed CSS boxes that
  // fence the texture away from type.
  const bloomRef = useDitherField(dawnArcs, {
    scale: 3,
    ink: '#f2f1ec',
    paper: 'transparent',
    fps: 20,
  ...
  const floorRef = useDitherField(sunburstFloor, {
    scale: 3,
    ink: '#f2f1ec',
    paper: 'transparent',
    fps: 20,
  ...
  return (
    <section className='tc-band tcb' id='toolchain' ref={root}>
      <div className='tcb-in'>
        ...
        <div className='tcb-grid'>
          {/* The centerpiece: the whole toolchain as one assembly. Planes and
              caption rows highlight each other on hover, focus, or tap. */}
      ...
      {/* C1.5 the floor: the sunburst rising from the band's bottom edge,
          mounted in the padding below every line of type. */}
