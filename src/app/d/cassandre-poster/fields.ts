...
export type PosterBurstOptions = {
  /** The fan's focus in field space, 0..1, `fy` measured from the top. */
  fx?: number;
  fy?: number;
  /** Direction the fan opens toward, radians (`-PI/2` is up). */
  dir?: number;
  /** Half-width of the lit sector around `dir`, radians. */
  spread?: number;
  /** Number of rays around the focus. */
  rays?: number;
  /** Distance from the focus at which the glow has faded out. */
  reach?: number;
  /** Ray rotation, radians per second; the still uses t = 0. */
  spin?: number;
  /** Peak coverage, 0..1. */
  gain?: number;
  /**
   * Vertical compression of the fan. Above 1 the fan flattens, so a wide
   * short panel can be lit edge to edge while the glow still fades out well
   * before the panel's far edge. Rays stay straight under the scaling.
   */
  squash?: number;
};

/**
 * POSTER BURST. The two poster panels' one tonal device, in the Bayer
 * engine: the fan of rays from the Cassandre school (the searchlight sky
 * behind the Normandie prow, the rails converging on the Étoile du Nord)
 * rendered as dither tiers instead of an airbrushed gradient. Coverage is
 * highest at the focus and falls off with distance; a cosine ray pattern
 * modulates it around the focus; a sector gate keeps the fan to one side
 * so the plate's far corners stay clean midnight for the captions. The
 * hero opens the fan upward from behind the monument's plinth; the dark
 * band opens it downward from above its head. Motion is a slow rotation
 * of the rays, one still under reduced motion.
 */
export function posterBurst(aspect: AspectBox, opts: PosterBurstOptions = {}): FieldFn {
  const {
    fx = 0.5,
    fy = 0.86,
    dir = -Math.PI / 2,
  ...
  return (u, v, t) => {
    const a = aspect.value;
    const dx = (u - fx) * a;
    const dy = (v - fy) * squash;
    const r = Math.sqrt(dx * dx + dy * dy);
    if (r >= reach) return 0;

    // the sector gate: angular distance from the fan's opening direction
    let da = Math.atan2(dy, dx) - dir;
    if (da > Math.PI) da -= TAU;
    if (da < -Math.PI) da += TAU;
    const sector = 1 - smoothstep(spread * 0.72, spread, Math.abs(da));
    if (sector <= 0) return 0;

    // the glow: bright at the focus, eased out toward the reach
    const g = 1 - smoothstep(0.02, reach, r);
    const glow = g * g;

    // the rays: a cosine comb around the focus, sharpened to thin spokes
    const ray = 0.5 + 0.5 * Math.cos((Math.atan2(dy, dx) + t * spin) * rays);
    const ray2 = ray * ray;
    const spokes = ray2 * ray2;

    // a hard band at the focus so the form stands on light, not on dust
    const plinth = 1 - smoothstep(0.03, 0.08, r);

    const field = gain * sector * glow * (0.28 + 0.72 * spokes);
    return field > plinth ? field : plinth;
  };
}
