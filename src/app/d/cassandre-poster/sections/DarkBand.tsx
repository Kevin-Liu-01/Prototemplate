...
import { floorDissolve, panelBloom, posterBurst, useDitherField } from '../fields';
...
export default function DarkBand() {
  ...
  /* The band's poster form: the fan of rays opening downward from above the
     head's top edge, flattened so it fades out by mid-height and the title
     below it stands on clean midnight. Cream ink read from the canvas color. */
  const fanRef = useDitherField(
    (aspect) =>
      posterBurst(aspect, {
        fx: 0.5,
        fy: -0.06,
  ...
  return (
    <section className='tc-band tcb' id='toolchain' ref={root}>
      <div className='tcb-in'>
        {/* The band's poster head: the fan behind, the setback mark, the title
            in the display face, and two mono captions pinned to the bottom
            corners. The fan is fenced to this block; no texture runs under the
            cells below. */}
        <div className='tcb-head cp-band-head' data-cell>
          <canvas className='cp-band-field' ref={fanRef} aria-hidden />
          <div className='cp-band-title'>
            <HeadMark />
            <h2>Everything you need, in one toolchain.</h2>
            <p>Buildtime, runtime, and review. One project, one config, one bill.</p>
          </div>
          <div className='cp-band-caps'>
            <span className='cp-band-cap'>buildtime · runtime · review</span>
            <span className='cp-band-cap'>128 strings · 6 locales · 640 translations</span>
          </div>
        </div>
        ...
        <div className='tcb-grid'>
          ...
          {/* The band's one light moment, in this fork's material: a real
              `gt translate` run floating in the dark core of the halftone
              bloom — the Bayer engine where toolchain runs its shader. */}
          <div className='tcb-cell tcb-cell-cli' data-cell>
            <canvas className='tcb-cli-field df-cli-bloom' ref={bloomRef} aria-hidden />
          ...
          <div className='tcb-cell tcb-cell-ctx' data-cell>
            <div className='tcb-cap'>
              <h3>Context, defined once and inherited all the way down</h3>
        ...
        <div className='tcb-acts' data-cell>
          ...
          <a className='tc-btn tc-btn-line' href='https://generaltranslation.com/enterprise/contact'>
            Talk to an Engineer
          </a>
        </div>
      </div>
