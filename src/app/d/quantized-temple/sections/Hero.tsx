...
import { templeSmoke, useDitherField } from '../fields';
import HeroCrown from './deco/HeroCrown';
...
/**
 * THE HERO, on the founder stack, in the temple's material.
 *
 * Two ranks of surfaces on the gilt mat, separated by 1px seams: a SPLIT row
 * (the copy card, with the crown above the h1, two authored headline lines
 * with the morphing `language` hinge, the sub with the GT wordmark inline,
 * the acts and the real copyable command) beside the PLATE, the hero's media
 * panel; then the trust card below, full width. On mobile the plate stacks
 * under the copy.
 *
 * The plate is the ziggurat: `templeSmoke` renders a five-course stepped form
 * whose smoke quantizes into ordered dither as it rises, solid at the base
 * course and loose dots at the summit, with a plume dying above it. The
 * field's ink is the canvas's computed `color` (the ornament token),
 * re-sampled when data-theme flips. Under the base course the accent runs
 * once as the ember line, the plate's one edge.
 *
 * The trust card is the shipped six-wordmark row, undithered: the 1-bit
 * process belongs to the plate, not to the customers' marks.
 */
...
export default function Hero() {
  ...
  /* The plate composition: the ziggurat filling the component. The ink is
     the ornament token through themeInk (the literal below is overwritten at
     mount). reducedMotionTime 5.2 is a composed still: smoke mid-drift, the
     plume half risen. */
  const canvasRef = useDitherField((aspect) => templeSmoke(aspect, { tiers: 5 }), {
    scale: 3,
    ink: '#070707',
    paper: 'transparent',
    fps: 24,
    ...
    <section className='tc-sec tch-hero-sec' id='top' ref={root}>
      ...
      <div className='tch-split'>
        <div className='tc-hero tch-card tch-copy'>
          {/* The crown: the monogram seated on its dithered plinth (C1.4). */}
          <HeroCrown />
          ...
          <p className='tc-hero-sub' data-hero-in>
            <span className='qt-wm'>
              <span className='qt-wm-mark'>
          ...
          {/* The acts carry the dev-first device too: the real, copyable
              command sits with the buttons, on the card, outside the plate,
              so the field keeps the whole component. */}
        ...
        {/* The plate: the hero's media panel, and the field fills ALL of it.
            The ziggurat stands on the ember line, the accent's one appearance
            in the hero. */}
        <div className='df-plate' data-hero-in>
          <canvas className='df-plate-field' ref={canvasRef} aria-hidden />
          <span className='df-plate-ember' aria-hidden />
        </div>
      </div>

      {/* The trust card: the six real wordmarks, ruled into six cells under
          one lead sentence. The claim is the lead; the restraint is the row. */}
