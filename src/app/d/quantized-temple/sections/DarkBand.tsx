...
import { panelBloom, summitDissolve, useDitherField } from '../fields';
import StepRule from './deco/StepRule';
...
export default function DarkBand() {
  ...
  // The band is the temple's summit (C1.5). The CLI cell's bloom and the
  // band's stepped floor are drawn by this page's own engine on permanently
  // dark plates; their ink is the ornament token, read from each canvas's
  // computed `color` through themeInk (the literals below are overwritten at
  // mount). applyStyles is off for both: the engine's default inline
  // `width/height: 100%` would override the fixed CSS boxes that fence the
  // texture away from type.
  ...
  const floorRef = useDitherField(summitDissolve, {
    scale: 3,
    ink: '#f2f1ec',
    paper: 'transparent',
    fps: 24,
  ...
  return (
    <section className='tc-band tcb' id='toolchain' ref={root}>
      ...
      {/* The floor: the summit's stepped horizon dissolving upward into dots,
          mounted in the band's bottom padding, below every line of type. */}
