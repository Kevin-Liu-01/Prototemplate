...
/**
 * QUANTIZED TEMPLE: the direction's field compositions.
 *
 * Everything visual on this page that is not type is a 1-bit Bayer field
 * rendered by `src/lib/dither.ts`. This module owns the direction's two
 * compositions (`templeSmoke`, the hero plate's ziggurat, and
 * `summitDissolve`, the dark band's stepped floor), keeps the fork's
 * `heroTransmission`, `panelBloom` and `floorDissolve` intact, and provides
 * the small React hook that mounts a `createDitherLoop` on a canvas and keeps
 * the field's aspect ratio honest across resizes.
 *
 * The compositional law applied to the thesis: the field is the armature, not
 * wallpaper. The ziggurat's silhouette is hard (a building) and its interior
 * is soft (smoke), and the ordered dither quantizes the difference: solid at
 * the base course, halftone through the middle courses, loose dots at the
 * summit.
 */
...
/* ------------------------------------------------------------------------ *
 * Value noise for the smoke. `dither.ts` keeps its own copy private, so the
 * three functions are restated here: an integer hash, its bilinear
 * interpolation, and a three-octave sum. Cheap enough to run per cell.
 * ------------------------------------------------------------------------ */
...
export type TempleSmokeOptions = {
  /** Courses in the ziggurat. Default 5. */
  tiers?: number;
};

/**
 * TEMPLE SMOKE: the hero plate's field.
 *
 * A stepped ziggurat centred on the plate: `tiers` courses of equal height,
 * each narrower than the one below, standing on a base course just above the
 * plate's bottom edge (the CSS ember line runs under it). Inside the
 * silhouette the fill is smoke, fractal value noise drifting upward, and its
 * coverage is tied to height: the base course prints solid, every course
 * above prints thinner, and the summit is loose dots. A one-cell seam at the
 * top of each course keeps the steps legible inside the smoke. Above the
 * summit a faint plume of the same noise rises, widens and dies before the
 * plate's top edge.
 *
 * Pure in (u, v, t): the reduced-motion still is a composed frame and
 * pause/resume can never drift.
 */
export function templeSmoke(aspect: AspectBox, opts: TempleSmokeOptions = {}): FieldFn {
  const TIERS = Math.max(3, opts.tiers ?? 5);
  /** The base course stands here; below it the plate stays paper. */
  const BASE_V = 0.975;
  /** The summit's height. */
  const TOP_V = 0.17;
  const BUILD_H = BASE_V - TOP_V;
  const TIER_H = BUILD_H / TIERS;
  /** Half-widths in u of the base and the top course. */
  const BASE_HW = 0.44;
  const TOP_HW = 0.1;

  return (u, v, t) => {
    if (v > BASE_V) return 0;
    const a = aspect.value;
    const h = BASE_V - v;
    const dx = Math.abs(u - 0.5);
    const smokeN = fbm(u * 3.1 * a + 0.4 * Math.sin(t * 0.07), v * 3.6 - t * 0.11, 3);

    if (v >= TOP_V) {
      const k = Math.min(TIERS - 1, Math.floor(h / TIER_H));
      const halfWidth = BASE_HW - (BASE_HW - TOP_HW) * (k / (TIERS - 1));
      if (dx > halfWidth) return 0;

      const hn = h / BUILD_H;
      // Density by height: solid at the foot, thinning to a few cells at the
      // crown. The smoke pushes it around by up to half its range.
      const dens = 1 - smoothstep(0.06, 1, hn) * 0.94;
      const smoke = clamp01(0.55 + (0.45 + 1.1 * (smokeN - 0.5)) * 0.75);
      let cov = dens * smoke;
      // The base course prints solid whatever the smoke does.
      const solid = 1 - smoothstep(0.02, 0.2, hn);
      if (solid > cov) cov = solid;
      // The course seam: a thin dark row where one tier meets the next.
      const inTier = (h - k * TIER_H) / TIER_H;
      if (inTier > 0.93) cov *= 0.3;
      return clamp01(cov);
    }

    // Above the summit: the plume.
    const rise = (TOP_V - v) / TOP_V;
    const spread = TOP_HW + rise * 0.22;
    if (dx > spread) return 0;
    const body = (1 - smoothstep(0, 0.85, rise)) * 0.16;
    const breath = clamp01(0.3 + 1.4 * (smokeN - 0.5));
    const edge = 1 - smoothstep(spread * 0.35, spread, dx);
    return clamp01(body * breath * edge);
  };
}

/**
 * SUMMIT DISSOLVE: the dark band's floor.
 *
 * The band is the temple's summit. Its floor strip carries a stepped horizon,
 * five courses narrowing upward, drawn as the boundary between paper above
 * and a coverage ramp below. Under the horizon the ramp thickens toward the
 * band's bottom edge, so the foot prints solid and the steps dissolve upward
 * into dots; above it nothing prints. A slow sway keeps the horizon
 * breathing. The strip is mounted in the band's bottom padding, so type never
 * enters it.
 */
export function summitDissolve(aspect: AspectBox): FieldFn {
  return (u, v, t) => {
    void aspect;
    const dx = Math.abs(u - 0.5);
    let horizon: number;
    if (dx < 0.08) horizon = 0;
    else if (dx < 0.18) horizon = 0.22;
    else if (dx < 0.3) horizon = 0.44;
    else if (dx < 0.42) horizon = 0.62;
    else horizon = 0.78;
    const sway =
      0.03 * Math.sin(u * TAU + t * 0.2) + 0.02 * Math.sin(u * 2.2 * TAU - t * 0.13);
    const d = v + sway - horizon;
    if (d <= 0) return 0;
    return clamp01(0.06 + d / (1 - horizon));
  };
}
