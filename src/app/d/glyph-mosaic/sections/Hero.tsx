...
import GlyphMosaic from './deco/GlyphMosaic';
import MosaicCrest from './deco/MosaicCrest';
import ScriptLegend from './deco/ScriptLegend';
...
/**
 * THE HERO, on the founder stack, recut for the mosaic.
 *
 * Two ranks of surfaces on the gilded grout, separated by 1px seams: a
 * SPLIT row (the sand copy card beside the mosaic plate) and the trust card
 * below, full width. The copy card opens with the crown (C1.4): a rising
 * sun of glyph tesserae over a baseline, then the lockup in the display
 * face, then the claim as one morphing shaped text node, the sub with the
 * GT wordmark inline, the two acts and the copyable command.
 *
 * The plate is the direction's statement: a sunburst mosaic whose tesserae
 * are real glyphs from seven scripts, laid where a radial field clears the
 * house Bayer screen, alternate rays in the ornament gold, one course in the
 * accent. A legend on the plate's bottom edge names each script and tags it
 * with the page's one locale chip. The plate is theme-following: the ink is
 * the canvas's computed color, re-read when data-theme flips.
 *
 * The trust card is the six customer wordmarks, monochrome, cap-height
 * aligned, in the ruled six-cell row.
 */
...
export default function Hero() {
  ...
  useGSAP(
    () => {
      ...
      /* The headline hinge is a measuring instrument. Each cycle: the bound
         ...
         the WHOLE SHAPED word, a hidden probe carrying the word's own
         lang/dir and the em's inherited type, never a sum of per-character
         boxes, which disconnects Arabic joining and splits Devanagari matras
         off their consonants. The em's width is the only layout-affecting
      ...
      if (em && word) {
        ...
        if (compactEvery) {
          ...
          const swap = () => {
            ...
            // 2. the word dissolves as ONE shaped run while the dust carries
            //    the scatter
            tl.to(word, {
              autoAlpha: 0,
              scale: 0.92,
            ...
            /* the cloud separates SYMMETRICALLY about the word's centre: each
               glyph takes an evenly-spread angle on a jittered ring */
            ...
            //    continuous tween, quantized to device pixels
  ...
  return (
    <section className='tc-sec tch-hero-sec' id='top' ref={root}>
      {/* The founder's stack, split: the sand copy card beside the mosaic
          plate, two cards on the gilded grout, one 1px seam between them;
          the trust row repeats the card grammar below. */}
      <div className='tch-split'>
        <div className='tc-hero tch-card tch-copy'>
          {/* C1.4: the crown. A rising sun of tesserae over its baseline,
              then the lockup in the display face. */}
          <div className='gm-crown' data-hero-in>
            <MosaicCrest className='gm-crown-crest' variant='rise' seed={3} />
            <span className='gm-crown-name'>General Translation</span>
          </div>

          {/* Two authored lines rather than a wrap; the morphing word opens
          ...
          <p className='tc-hero-sub' data-hero-in>
            <span className='gm-wm'>
              ...
              <Image
                ...
                height={22}
              />
              <span className='gm-vh'>General Translation</span>
            </span>
          ...
          {/* The acts carry the dev-first device too: the real, copyable
              command sits with the buttons, on calm sand, outside the plate. */}
        ...
        {/* C1.4: the plate. The sunburst mosaic fills the field edge to edge;
            the legend names the seven scripts on the plate's bottom edge. */}
        <div className='gm-plate' data-hero-in>
          <div className='gm-plate-field'>
            <GlyphMosaic
              variant='sunburst'
              className='gm-plate-canvas'
              cell={14}
              seed={1}
              title='A sunburst mosaic whose tesserae are glyphs from Latin, Greek, Cyrillic, Arabic, Devanagari, Hangul and Han scripts, alternate rays in gold, one course in terracotta around the disc'
            />
          </div>
          <ScriptLegend />
        </div>
      ...
      {/* The trust card: six wordmarks, monochrome, in the ruled six-cell row.
          The claim is the lead; the restraint is the diagram. */}
