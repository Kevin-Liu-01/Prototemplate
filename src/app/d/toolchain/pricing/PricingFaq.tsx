...
/**
 * The FAQ as a ruled ledger: question in ink on the left column, answer in
 * the muted step on the right, one hairline per row, edge to edge. No
 * accordions — the still frame carries the whole argument.
 */
export default function PricingFaq() {
  ...
  return (
    <section className='tc-sec' id='faq' ref={root}>
      ...
      <dl className='tcp-faq'>
        {FAQ.map((row) => (
          <div className='tcp-faq-row' data-reveal key={row.id}>
            <dt>{row.q}</dt>
            <dd>{row.a}</dd>
          </div>
        ))}
