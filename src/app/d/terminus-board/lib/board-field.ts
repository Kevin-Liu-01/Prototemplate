...
export function createBoardField(
  ...
  const fontNoise = `${fontPx(0.5)}px ${MONO}`;
  const fontCode = `${fontPx(0.38)}px ${MONO}`;
  const fontWord = `500 ${fontPx(0.6)}px ${MONO}`;
  const fontStamp = `500 ${fontPx(0.4)}px ${MONO}`;
  ...
  function buildStatic(): void {
    ...
    const picks: Array<{ run: [number, number] | undefined; word: BoardWord | undefined; bias: number }> = [
      ...
      { run: bottom, word: WORDS[3], bias: 0.62 },
    ];
