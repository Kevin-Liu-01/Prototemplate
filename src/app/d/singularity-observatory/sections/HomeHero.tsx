...
/* ------------------------------------------------------------------
   The gt CLI session as a staged pipeline. Three faces the reader can
   walk: 01 install (the wizard detects the framework and writes the
   config), 02 translate (`gt translate` scans, translates per string),
   03 serve (the payload gt wrote, addressed the way a request hits it).
...
/* ------------------------------------------------------------------
   The serve stage's artifact: the hashed-key JSON `gt translate`
   ...
   structure and only the leaves change). One locale carries the
   argument: a request to example.com/es is answered from this file.
   ------------------------------------------------------------------ */
...
/* the three stages of the pipeline: numbers are mono, words are sans */
type Stage = 'install' | 'translate' | 'serve';

const STAGES: readonly { id: Stage; num: string; word: string }[] = [
  { id: 'install', num: '01', word: 'install' },
  { id: 'translate', num: '02', word: 'translate' },
  { id: 'serve', num: '03', word: 'serve' },
];
...
export default function HomeHero() {
  ...
  /* the pipeline's three faces */
  const [stage, setStage] = useState<Stage>('install');
  /* One guided pass walks the stages in order; the first manual interaction
     kills it for good — the reader has taken the controls. */
  ...
  useGSAP(
    () => {
      if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        /* every animation below is a `from`, so the still already shows the
           finished install transcript — nothing to park */
        return;
      }
      ...
      /* ---- the terminal session replays ----
         ...
         timing and all. Some targets live in faces that are hidden at replay
         time — their `from` tweens still land, so each face is complete the
      ...
      /* ---- one guided walk down the pipeline ----
         After the replay settles — and after the finished install transcript
         has owned the window long enough to be the first fold's still — the
         window walks the stages once, install → translate → serve, then
         hands the prompt back. Any manual stage click kills this timeline
         before it acts. */
      const guided = gsap.timeline({ delay: run.duration() + 1.6 });
      guided
        .to({}, { duration: 2.4 })
        .call(() => setStage('translate'))
        .to({}, { duration: 3.2 })
        .call(() => setStage('serve'))
        .to({}, { duration: 2.6 })
        .call(() => {
      ...
      <div className='tc-hero-cell tch-band'>
        ...
        <div className='tct-win' data-hero-in>
          <div className='tct-bar'>
            <span className='tct-title'>gt — pipeline</span>
            <div className='tct-seg sgbh-stageseg' role='group' aria-label='Pipeline stage'>
              {STAGES.map((s) => (
                <button type='button' data-on={stage === s.id} key={s.id} onClick={() => pickStage(s.id)}>
                  <i className='sgbh-num'>{s.num}</i>
                  <span className='sgbh-word'>{s.word}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className='tct-stage'>
            {/* stage 01 — install: the wizard detects and writes the config */}
            <div className='tct-face sgbh-face' data-on={stage === 'install'} aria-hidden={stage !== 'install'}>
            ...
            {/* stage 02 — translate: the scan, the per-string table, the timing */}
            <div className='tct-face sgbh-face' data-on={stage === 'translate'} aria-hidden={stage !== 'translate'}>
              <div className='tct-body'>
            ...
            {/* stage 03 — serve: the file gt wrote, addressed the way a
                request hits it — example.com/es answered from _gt/es.json */}
            <div className='tct-face sgbh-face' data-on={stage === 'serve'} aria-hidden={stage !== 'serve'}>
              <div className='tct-body'>
                <div className='tct-line tct-meta' data-td>
                  {'  Wrote public/_gt/'}
                ...
                <div className='tct-gap' />
                <div className='sgbh-artifact'>
                  <div className='sgbh-addr'>
                    <span>
                      example.com/<b>es</b>
                    </span>
                    <span className='sgbh-file'>public/_gt/es.json</span>
                  </div>
                  <div className='sgbh-payload'>
                    <PayloadJson loc='es' />
                  </div>
                </div>
              </div>
