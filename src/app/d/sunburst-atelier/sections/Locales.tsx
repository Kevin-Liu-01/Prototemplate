 ...
 *
 * Beside the table, the atlas: the halftone globe is the hub of one shared
 * sunburst, and eight locale chips sit at the ends of its rays. Every chip is
 * a real tag through LocaleTag; the rays are leaders, not ornament.
 */
...
/* The eight tags on the rays: a spread of scripts and regions, every one a
   locale the platform serves and every one with a flag in LocaleTag's map. */
const RAY_LOCALES: readonly string[] = ['es', 'fr', 'de-CH', 'ja', 'zh-Hant', 'ar-SA', 'pt-BR', 'hi'];
      ...
      <div className='tc-row is-lead'>
        <div className='tc-cell is-tall' data-reveal>
          ...
          <div className='tc-cell-act'>
            <a className='tc-btn tc-btn-line tc-btn-sm' href='https://generaltranslation.com/docs'>
        ...
        <div className='tc-cell is-tall is-framed' data-reveal>
          <div className='tc-card'>
            {/* The atlas: a square well, the globe at the hub, the rays and
                their chips around it. The canvas is the functional image; the
                rays overlay is a labeled diagram of the tags it serves. */}
            <div className='df-globe-well'>
              <div className='sba-atlas'>
                <DitherGlobe title='A halftone globe with twelve meridians and seven parallels, turning slowly, with eight locale tags on rays around it' />
                <AtlasRays locales={RAY_LOCALES} />
              </div>
            </div>
            {/* Under 720px the ray overlay hides and the same eight chips
                reflow as one row beneath the globe. Exactly one of the two
                sets is rendered at any width, so neither is aria-hidden. */}
            <div className='sba-atlas-list'>
              {RAY_LOCALES.map((code) => (
                <span className='sba-ray-tag' key={code}>
                  <LocaleTag code={code} />
                </span>
              ))}
            </div>
