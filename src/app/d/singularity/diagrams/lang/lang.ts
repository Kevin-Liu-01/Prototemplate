...
export function langClass(variant: string, accent: boolean, className?: string): string {
  return ['lang', variant, accent ? 'lang-accent-on' : 'lang-accent-off', className].filter(Boolean).join(' ');
}
