 ...
 * orthogonal trace at constant gauge — a circuit run, not string art — and
 * it appears whole (an alpha fade over fixed geometry), so no scrub position
 * can sample a half-drawn wire. Captions swap atomically, panels arrive
 * whole, and the dark surface spends its one material moment under the page
 * while the translation sweeps it.
 */
...
/** A ghost wrapper glyph a beat draws around a node — zero-width anchor,
    so the site never reserves space for it. `lift` raises a wide opener
    above its own line instead of past the panel edge; `above` hangs the
    context beat's opener over the tagline card, clear of the ring. */
function Wrap({
  n,
  t,
  side,
  ...
  return (
    <i
      className={`tc-cinema-wrapmark is-${side}${lift ? ' is-lift' : ''}${above ? ' is-above' : ''}`}
      data-wrap={n}
      aria-hidden
    >
      <span>{t}</span>
  ...
  useGSAP(
    () => {
      ...
      /* The whole composition lands inside the beat's first tenth — the
         trace appears whole (never sampled half-drawn), the ring and the
         ghost context tag materialize with it, so every scrub position
         in the band shows the finished still. */
      ft(wCtx, { autoAlpha: 0 }, { autoAlpha: 1, duration: 0.5 }, 8.1);
      frame("[data-node='tagline']", 8.2, 7);
      ft(
        "[data-wrap='ctx']",
        { autoAlpha: 0, x: 5 },
      ...
      tl.to("[data-wrap='ctx']", { autoAlpha: 0, duration: 0.4 }, 16.8);
      ...
      /* ================= BEAT 5 · review (56–66) ================= */
      cap(5, 56);
      ft(wRev, { autoAlpha: 0 }, { autoAlpha: 1, duration: 0.5 }, 56.1);
      frame("[data-node='legal']", 56.2, 5);
          ...
          <div className='tc-cinema-demo' data-page>
            ...
            <div className='tc-cinema-drow is-card'>
              <div className='tc-cinema-dcard' data-node='tagline'>
                <Wrap
                  n='ctx'
                  t='<T context="Playful, upbeat marketing tone">'
                  side='open'
                  above
                />
                <b>
                  <Sw
                    ...
                    tail={<Wrap n='ctx' t='</T>' side='close' />}
                  />
                </b>
