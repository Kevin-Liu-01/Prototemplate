...
/**
 * The page's one full-bleed dark band. The light is composed around the
 * band's own object (AESTHETIC_ADDENDUM 2b): the burst's core sits directly
 * behind the `gt status` transcript — the band's meaning-charged artifact —
 * and decays to pure ink well before the frame edges, so the corners are the
 * darkest thing in the band instead of the brightest. The transcript stands
 * in front of the light the way the hero's `fra` tick stands in the burst's
 * eye: shader and information are one object.
 */
export default function DarkBand() {
  ...
  return (
    <section className='tc-band' id='toolchain' ref={root}>
      {/* Mask lives on a band-sized wrapper (not the oversized canvas), so
          the halo's percent anchors are the band's own geometry. */}
      <div className='tc-band-light' aria-hidden>
        <PrismaticField
