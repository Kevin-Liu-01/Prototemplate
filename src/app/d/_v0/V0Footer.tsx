...
/* The compliance program, as house marks: hairline-outlined stamps, all
   three doors into the same trust center. Cert identifiers carry numerals,
   so they ride in mono under the numbers rule. */
const BADGES: readonly { label: string; detail?: string }[] = [
  { label: 'SOC 2', detail: 'AICPA' },
  { label: 'GDPR' },
  { label: 'ISO 27001' },
];

/**
 * The v0 close: the production footer's full contents on one ruled sheet.
 * Brand + compliance stamps in the left cell, five link columns beside it —
 * the grid owns every seam — then a closing strip with live status, the
 * copyright line, and the theme switch. Both themes render it; square
 * corners, hairlines only.
 */
    ...
    <footer className='tc-sec v0-foot'>
      <div className='v0-foot-grid'>
        <div className='v0-foot-cell v0-foot-brand'>
          ...
          <Image
            className='v0-foot-logo is-light'
          ...
          />
          <Image
            className='v0-foot-logo is-dark'
          ...
          <div className='v0-foot-badges'>
            {BADGES.map((badge) => (
              <a
                aria-label={`${badge.label} compliance — General Translation trust center`}
                ...
                target='_blank'
              >
                <ShieldCheck aria-hidden size={12} strokeWidth={1.8} />
                <span>{badge.label}</span>
                {badge.detail ? <em>{badge.detail}</em> : null}
              </a>
            ))}
          </div>
        </div>
