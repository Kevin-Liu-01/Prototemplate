...
/* ------------------------------------------------------------------
   ...
   whole so any home can mount it. One `GT · Translate` window, two
   faces behind a seg: the terminal face replays the real gt CLI run,
   the preview face renders the run's strings as the localized product
   page they ship to — locale tabs, per-line rewrite, inspector marks
   bridging each component to its payload key, the slide-to-reveal
   seam over public/_gt/[locale].json, and a live JSON↔UI edit loop.
   The window is self-contained: its own state, timelines and styles
   ride along (hero-terminal.css is the shared window sheet; the
   window-only rules live in translate-window.css under the .v0-tw
   root class this component adds).
   ------------------------------------------------------------------ */
  ...
  return (
    <div className='tct-win v0-tw' data-hero-in ref={root}>
      <div className='tct-bar'>
        {/* the standalone brand occurrence speaks the drawn mark, not the
            letters (founder addendum): the plate is permanently dark, so
            the dark-theme (light-ink) asset is the one right choice */}
        <span className='tct-title'>
          <Image src='/brand/no-bg-gt-logo-dark.png' alt='GT' width={14} height={14} />
          <span>Translate</span>
        </span>
      ...
      {/* the stack strip: window furniture, directly under the bar — the
          ...
          id='frameworks' because it IS its page's frameworks content:
