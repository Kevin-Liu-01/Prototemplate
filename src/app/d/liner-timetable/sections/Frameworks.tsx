...
import PlateNumeral from './deco/PlateNumeral';
...
/** The locale each CAP_DEMOS output is actually in, where it is one: the
    ...
    the Polish plural pair. The demo gets the page's one locale chip, LocaleTag, flag + mono code, beside it; a bare flag is ambiguous
 ...
 */
function CapMarquee({ caps, label }: { caps: readonly string[]; label: string }) {
  ...
  return (
    <div className='tc-caps is-marquee' ref={wrap}>
      {/* The board's bar mirrors the code window's: the board's name, then the
          SDK whose outputs it lists. */}
      <div className='tc-caps-bar'>
        <span>arrivals</span>
        <span>{label}</span>
      </div>
      <div className='tc-caps-body'>
...
export default function Frameworks() {
  ...
  useGSAP(
    () => {
      ...
      if (!list || !bar) return;
      /* Under reduced motion the underline is placed, never slid, and the
         panel swaps without its fade: the markup pose is the still. */
      ...
      const place = (animate: boolean) => {
        ...
        if (animate && !reduce) gsap.to(bar, { ...to, duration: 0.42, ease: 'power3.out' });
        else gsap.set(bar, to);
      };

      place(settled.current);

      if (settled.current && panel.current && !reduce) {
        gsap.fromTo(
          panel.current,
          { autoAlpha: 0, y: 6 },
      ...
      >
        ...
        <div className='tc-fw-code'>
          <div className='tc-mount'>
            <div className='tc-card'>
              <CodeBlock file={framework.file} code={framework.code} label='departures' />
            </div>
          </div>
        </div>
        ...
        <div className='tc-fw-side'>
          ...
          {/* Each capability row carries a real output beside its name, the
              ...
              beside them. */}
          <CapMarquee caps={framework.caps} label={framework.pkg} />

          <a className='tc-fw-link' href='https://generaltranslation.com/docs'>
