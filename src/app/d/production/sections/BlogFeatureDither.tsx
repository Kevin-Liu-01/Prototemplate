 ...
 *
 * Ported from apps/landing/src/components/blog/BlogFeatureDither.tsx.
 * The only change: the shipped file derives its 8×8 Bayer matrix from the
 * landing app's BAYER4 at module load; this repo already ships the same
 * matrix pre-built as BAYER_8 in src/lib/dither.ts (verified cell for cell
 * against that derivation), so the ramp reads it rather than rebuilding it.
 */
...
/** SVG font-size that fills the 400×225 plate for this word: bucketed
    by length, sized so the longest package names still fit the plate. */
...
/** Returns a post's fallback cover motif from its title, tags, and slug. */
...
/** Coverage tiers of 64, solid-side first (the per-slug covers' ramp,
    scaled to cross a full plate). The dither opens early and steps in
    small, even increments so the solid-to-dither handoff reads as one
...
function centerYFor(word: string, fontSize: number): number {
  ...
  if (/^gt[-x]/.test(word) && fontSize >= 75) {
    return 112.5 - fontSize * 0.14;
  }
