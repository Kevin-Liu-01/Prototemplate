...
/**
 * One layer of the stack: the eyebrow tag (icon + NAME), the h3 value
 * prop, and bullet-point copy. The points are ReactNodes, not strings, so
 * each can carry inline markup where the copy needs it. Slab membership is
 * by TOWER_LAYERS id, never by index, so the mapping survives any
 * reordering of the house drawing.
 */
type StackBeat = {
  id: string;
  name: string;
...
/**
 * The four layers, ordered bottom-up: Code -> Context -> Translations
 * -> Agents. The tower draws these same four layers, one slab per beat,
 * so beat order IS physical order.
 */
const BEATS: readonly StackBeat[] = [
  {
    ...
    points: [
      ...
      'Open-source internationalization (i18n) libraries and SDKs for every major framework',
    ],
    icon: Code2,
    slabIds: ['code'],
  },
  {
    ...
    points: [
      ...
      'Share context, custom prompts, and glossaries across every product surface',
    ],
    /* BookOpen, not Layers: Layers is the FULL STACK's icon; context
       wears the knowledge glyph everywhere */
    icon: BookOpen,
    slabIds: ['context'],
  },
  {
    ...
    points: [
      'Localize copy and user-generated content in native-level quality',
          ...
          <div className='tcb-cell v0-stack-cell-rail' data-cell>
            ...
            <ol className='v0-stack-rail'>
              ...
              {/* the agents dwell's runway, as structural flow INSIDE the
                  ...
                  PARENT'S content box, so this spacer is the room the
                  finale's copy spends staying with the viewer (~260px)
                  while the scan beam plays. */}
