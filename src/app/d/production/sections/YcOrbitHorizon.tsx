...
type OrbitHorizonProps = {
  /* scene/canvas classes are passed whole — the pages' middle word
     differs (careers-horizon-* vs yc-hole-*) — while the orbit classes
     and hole CSS vars share one shape: `${classPrefix}-orbit*` and
     `--${classPrefix}-hole-*`. All must match the page CSS exactly. */
  className: string;
  canvasClassName: string;
  classPrefix: string;
  orbitRadiusScale: number;
  redshiftDuration: number;
  fieldSpeed: number;
  /* mobile ring width cap, as a fraction of the host width */
  mobileWidthFactor: number;
  /* vertical center of the hole on wide layouts, as a fraction of height */
  wideCenterYFactor: number;
  /* canvas half-extent as a multiple of the hole radius */
  canvasHalfScale: number;
};
...
/**
 * OrbitHorizon, reproduced.
 *
 * 1-1 with apps/landing/src/components/landing/shared/OrbitHorizon.tsx —
 * the shipped yc hero's object. The shared horizon-field shader draws the
 * event horizon into a canvas sized off the measured hole, thirty locale
 * words ride the ring tangent to its arc (each letter rolling over as it
 * crosses the far side, the flag sweeping ahead of it), the doppler flank
 * walks once round every `redshiftDuration` seconds, and the resolved
 * geometry is published back to the section as --{prefix}-hole-x/y/radius
 * so the copy, the ruled mask and the ink disc all read the same numbers.
 *
 * Two substitutions, both forced by what this repo has:
 *   - the flag is a flag-icons span through this repo's own localeFlag()
 *     map instead of the design system's LocaleFlag component — same
 *     `fi fi-<region>` artwork, same box, same data-orbit-part hook;
 *   - the scene's accessible name is the English string the real page
 *     passes through gt().
 */
export default function YcOrbitHorizon({
  className,
  canvasClassName,
  classPrefix,
  orbitRadiusScale,
  ...
  useGSAP(
    () => {
      ...
      const wordElements = Array.from(
        orbit.querySelectorAll<HTMLElement>(`.${classPrefix}-orbit-word`)
      );
      let orbitRadius = 320;
      ...
      const words: OrbitWord[] = wordElements.map((wordRoot) => {
        ...
        const letters = parts.filter(
          (part) => !part.classList.contains(`${classPrefix}-orbit-flag`)
        );
        const flagElement = parts.find((part) =>
          part.classList.contains(`${classPrefix}-orbit-flag`)
        );
        return {
          root: wordRoot,
      ...
      const fit = () => {
        ...
        const orbitV = wide
          ? radius * orbitRadiusScale * WIDE_ORBIT_TILT + wordPad
          : Math.min(radius * 1.15, width * mobileWidthFactor) *
              MOBILE_ORBIT_TILT +
            wordPad;
        const centerX = width / 2;
        const centerY = Math.max(
          Math.min(
            height * (wide ? wideCenterYFactor : 0.46),
        ...
        orbitRadius = wide
          ? radius * orbitRadiusScale
          : Math.min(radius * 1.15, width * mobileWidthFactor);
        packWords();

        const half = radius * canvasHalfScale;

        frame.style.setProperty(`--${classPrefix}-hole-x`, `${centerX}px`);
        frame.style.setProperty(`--${classPrefix}-hole-y`, `${centerY}px`);
        frame.style.setProperty(`--${classPrefix}-hole-radius`, `${radius}px`);
      ...
      const tick = () => {
        ...
        placeWords(time);
        field?.setParams({
          dopplerAngle: REDSHIFT_START_ANGLE + (time / redshiftDuration) * TAU,
        });
      };
  ...
  return (
    <div
      className={`${className} orbit-hole-scene`}
      ref={root}
      role='img'
    ...
    >
      ...
      <div className='orbit-hole-placeholder' aria-hidden='true' />
      <canvas
        className={`${canvasClassName} orbit-hole-canvas`}
        ref={canvasRef}
        aria-hidden='true'
      />
      <div className={`${classPrefix}-orbit`} ref={orbitRef} aria-hidden='true'>
        {LOCALE_ORBIT.map((entry) => {
          const flag = localeFlag(entry.locale);
          return (
            <span className={`${classPrefix}-orbit-word`} key={entry.name}>
              {flag ? (
                <span
                  aria-hidden='true'
                  className={`fi fi-${flag} ${classPrefix}-orbit-flag`}
                  data-orbit-part
                />
              ) : null}
              {entry.whole ? (
