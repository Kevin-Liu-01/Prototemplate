...
/**
 * The global/enterprise section, reproduced from the shipped page: a tc-head
 ...
 * a framed cell stacking the three claims as a ruled rail — and the Theo
 * quote plate as one full-width cell. Only the artifacts inside the cells
 * (globe mount, locale panel, quote type) are styled here.
 */
...
/** Base languages fanning into regional locale tags. */
const FAMILY_ROWS: readonly FamilyRow[] = [
  {
    base: 'en',
    native: 'English',
    variants: [{ code: 'en-GB' }, { code: 'en-AU' }, { code: 'en-CA' }],
  },
  {
    base: 'es',
    native: 'Español',
    variants: [{ code: 'es-MX' }, { code: 'es-AR' }],
  },
  {
    base: 'zh',
    native: '中文',
    variants: [{ code: 'zh-HK' }, { code: 'zh-TW' }],
  },
];

/** The script belt: five more writing systems close the panel. */
  ...
  return (
    <section className='tc-sec v0-glob' id='infrastructure' ref={root}>
      <div className='tc-head'>
        ...
        <h2 data-reveal>Reach users in every market</h2>
        <p data-reveal>
          Grow your product around the world and reach your next billion users
        </p>
      </div>
          ...
          <div className='v0-glob-rail'>
            <article className='v0-glob-item'>
              ...
              <div className='v0-glob-locales'>
                {FAMILY_ROWS.map((row) => (
                  <div className='v0-glob-lrow' key={row.base}>
                      ...
                      {row.variants.map((variant, i) => (
                        <Fragment key={variant.code}>
                          ...
                          <span className='v0-glob-lvar'>
                            <code className='v0-glob-chip'>
                              <LocaleTag code={variant.code} />
                            </code>
                          </span>
                        </Fragment>
                      ))}
                  ...
                  {SCRIPT_BELT.map((token, i) => (
                    <Fragment key={token.code}>
                      ...
                      <span className='v0-glob-lvar'>
                        ...
                        <span
                          className='v0-glob-native'
                          dir={token.code === 'ar' ? 'rtl' : undefined}
                          lang={token.code}
                        >
                          {token.native}
                        </span>
            ...
            <article className='v0-glob-item'>
              ...
              <p>
                Store translations alongside your code or in a low-latency CDN,
                versioned by locale
              </p>
            </article>
            ...
            <article className='v0-glob-item'>
              ...
              <p>
                Dedicated FDE hours to build any workflow across code and
                content. Plus SSO, SOC 2 Type II, ISO 27001, and audit logs
              </p>
            </article>
          </div>
        </BentoCell>
      ...
      {/* ---- row 2: the quote plate in pricing-v2.css's tcpq grammar —
          oversized mark, accented phrases, avatar attribution — compressed
          vertically by the v0 overrides in global.css. */}
          ...
          <figure className='tcpq-mat'>
            <div className='tcpq-plate'>
              ...
              <blockquote className='tcpq-quote'>
                <p>
                  Every once in awhile, I see a snippet of code that makes me a
                  bit emotional.
                </p>
                <p>
                  Now is one of those moments. Internationalization went from{' '}
                  <em>&ldquo;$%!# this&rdquo;</em> to{' '}
