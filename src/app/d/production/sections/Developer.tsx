...
/* The shipped page routes this through generaltranslation's formatDateTime,
   which is Intl.DateTimeFormat pinned to the Gregorian calendar and Latin
   digits with no dateStyle — i.e. each locale's short numeric form
   (8/4/2026 · 4.8.2026 · 2026/8/4). Inlined here: this repo has no
   generaltranslation dependency, and the output must stay identical. */
...
const FORKS: readonly ForkSpec[] = [
  {
    ...
    source: '1234567.89',
    branches: [
      { code: 'en', value: formatNumber('en-US') },
      { code: 'de', value: formatNumber('de-DE') },
      { code: 'fr', value: formatNumber('fr-FR') },
    ],
    featured: 2,
    title: 'The number 1234567.89 forking to its en-US, de-DE and fr-FR renderings',
  },
  {
    label: 'Currency',
    icon: Coins,
    source: '1234.5',
    branches: [
      { code: 'en', value: formatCurrency('en-US', 'USD') },
      { code: 'de', value: formatCurrency('de-DE', 'EUR') },
      { code: 'ja', value: formatCurrency('ja-JP', 'JPY').normalize('NFKC'), lang: 'ja' },
    ],
    featured: 2,
    title: 'The amount 1234.5 forking to US dollar, euro and Japanese yen renderings',
  },
  {
    label: 'Dates',
    icon: CalendarDays,
    source: '2026-08-04',
    branches: [
      { code: 'en', value: formatDate('en-US') },
      { code: 'de', value: formatDate('de-DE') },
      { code: 'ja', value: formatDate('ja-JP'), lang: 'ja' },
    ],
    featured: 2,
        ...
        return (
          ...
          >
            <div className='v0-dev-fork-body'>
              ...
              {/* three equal rows pin the landings at 1/6, 1/2 and 5/6 of the
                  drawing; basis auto lets a two-line plural row lift all three */}
              <div className='v0-dev-outs'>
                {spec.branches.map((branch) => (
                  <div
                    className={branch.note ? 'v0-dev-out has-note' : 'v0-dev-out'}
                    data-dev-out=''
                    key={branch.code}
                  >
...
export default function V0Developer({ heading }: V0DeveloperProps) {
    ...
    <section className='tc-sec v0-dev' id='developers' ref={root}>
      <div className='tc-head'>
        ...
        <h2 data-reveal>
          {heading ?? (
            <>
              Built for developers <br className='v0-dev-title-break' />
              around the globe
            </>
          )}
        </h2>
        <p data-reveal>
          <GtLogoText /> handles all the localization infrastructure, so you can
          focus on shipping a product the world loves
        </p>
      </div>

      {/* 3 → 2 → 1: at two columns the routing ledger takes the full second
          row; stacked the clearance goes under the heads */}
      <div className='tc-row v0-dev-grid' data-eq-heads=''>
        ...
        <BentoCell cell='is-framed' title='Search-optimized routing for any locale'>
          {/* one hairline between neighbours, nothing at the ends; the rows
              spread over the cell's height */}
