...
/**
 * Split-flap animation engine for DOM text. Vendored from the shipped
 * landing app (apps/landing/src/components/blog/flap.ts); the only change
 * is the flash fallback noted below.
 *
 * React renders characters as two-layer cells (see FlapText.tsx);
 * this module mutates only the
...
/* PORT NOTE: the shipped engine keeps a literal hex here as the settle
   flash's last resort. This repo bans raw hex in .ts/.tsx, so the chain
   falls back to the direction's own accent token and then to the face's
   own ink (no flash) instead. Both sheets define --tb-flash, so the
...
export function flipUp(
  ...
  faces(line).forEach((face, i) => {
    ...
    const accent =
      style.getPropertyValue('--tb-flash').trim() ||
      style.getPropertyValue('--tc-accent').trim() ||
      ink;
