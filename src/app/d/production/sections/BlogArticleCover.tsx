...
/**
 * PRODUCTION · the post's cover — a port of the shipped BlogPostCover and
 * the BlogFeatureDither it falls back to (apps/landing/src/components/blog).
 *
 * A post whose frontmatter names an image gets that image. Everything else
 * gets the motif plate: the post's own word, cut as an SVG mask over a
 * tiered Bayer ramp, so the glyphs render as dither cells that thin from
 * solid coverage on one flank to sparse on the other. Server-rendered SVG —
 * no canvas, no image, nothing to load.
 *
 * The ink is set in blog-article.css (`pattern path`), not here: this repo
 * bans raw hex in tsx, and the ramp has to re-skin in the dark.
 */
...
export type MotifSource = {
  ...
  /** Frontmatter `images` — the first entry becomes the cover. */
...
export default function BlogArticleCover({
  post,
  id,
  className,
  scale = 1,
  ...
  /* The frontmatter path carries a cache-buster (`?v=…`), kept verbatim in
     the vendored record. next/image refuses a local src with a query string
     unless next.config declares images.localPatterns, which is shared
     config; the query addresses nothing on this asset, so it is dropped on
     the way to the loader rather than configured around. */
  const image = post.images?.[0]?.split('?')[0];
  if (image) {
    return (
      <Image
        alt=''
        className={`pba-dither pba-photo${className ? ` ${className}` : ''}`}
        height={540}
        priority={priority}
        sizes={sizes}
        src={image}
  ...
  return (
    <span
      aria-hidden='true'
      className={`pba-dither${className ? ` ${className}` : ''}`}
    >
