...
import '../../toolchain/sections/review-workspace.css';

gsap.registerPlugin(useGSAP, ScrollTrigger);

/* PRODUCTION reproduction of the shipped landing page's ReviewWorkspace
   (apps/landing/src/components/landing/sections/context/ReviewWorkspace.tsx).
   It is section-local rather than the shared toolchain mount because the
   toolchain copy carries two older rows ("Launch in every language", the
   "End-to-end localization" meta line); the shipped table says "Scale to
   every language" and "Full-stack localization". The shared sheet and the
   shared reveal hook are imported, not copied — only the strings and markup
   are reproduced here. gt-next's <T>/gt()/msg() resolve to their en-US
   strings. */
...
/** The regenerated row, hoisted so the API and CLI faces show the SAME
    edit the table does. */
const META_ROW: ReviewRow = {
  key: 'meta',
  source: "Full-stack localization for the world's best companies",
...
/**
 * The review rows, all real GT copy: the demo app's canonical string, the
 * hero headline, the meta description (the regenerated row) and the demo's
 * terms line — each beside its Spanish translation.
 */
const ROWS: readonly ReviewRow[] = [
  ...
  {
    key: 'hero',
    source: 'Scale to every language',
    translation: 'Crece en todos los idiomas',
    final: 'approved',
  },
  META_ROW,
  {
...
type ReviewWorkspaceProps = {
  /** Left-card copy override — hosts swap the words, never the
      workspace itself. */
  heading?: string;
  ...
  /** 'product' dresses the frame as the dashboard itself: lucide glyphs in
      the bar and footer, and the two aura hooks the host styles into a
      dither field around the mat. 'terminal' (the default) renders the
      plain terminal chrome. */
  chrome?: 'terminal' | 'product';
  /** Renders a rail of three door nodes wired into the frame, each
      swapping its face to that surface's real text — the workspace table,
      the diffs call, the save-local session. Off by default; without it
      no rail and no faces render. */
  surfaces?: boolean;
};
...
export default function ReviewWorkspace({
    ...
    () => {
      ...
      ROWS.forEach((row, i) => {
        ...
        const stampIn = (at: number) => {
          ...
          tl.call(
            setStamp(row.final === 'approved' ? 'approved' : 'edit', finalKind),
            undefined,
  ...
  /* The window's resting face — the workspace table with its bar and
     status strip. Surfaces mode wraps it in a face so the API and CLI
     stills can cover it; without surfaces it renders unwrapped. */
  const webBody = (
    <>
              ...
              {ROWS.map((row) => (
                <Fragment key={row.key}>
                  ...
                  <div className='tcr-cell is-t' data-state={row.previous ? 'revised' : 'clean'}>
                    ...
                    >
                      {row.final === 'approved' ? 'approved' : 'edit'}
                    </span>
                  </div>
                </Fragment>
              ))}
