...
/**
 * The shipped 404's horizon, reproduced.
 *
 * Ported verbatim from the real page's NotFoundHorizon
 * (apps/landing/src/components/pages/not-found/NotFoundHorizon.tsx): the
 * shared horizon-field shader fitted so the whole GLOW — not merely the
 * disc — clears the band, then handed a slow redshift sweep that walks the
 * bright doppler flank once around the ring every 210s. The measured
 * geometry is published back to the section as --pnf-hole-x/y/radius (the
 * real sheet's --not-found-hole-*), which the copy, the ruled mask and the
 * ink disc all read, so nothing has to guess where the hole landed.
 *
 * The only edit against the original is the gt-next `useGT()` aria label,
 * which this repo renders as the plain English string.
 */
export default function NotFoundHorizon() {
  ...
  useGSAP(
    () => {
      ...
      const themedParams = () =>
        doc.getAttribute('data-theme') === 'dark' ? HORIZON_DARK : HORIZON_LIGHT;
      ...
      const fit = () => {
        ...
        frame.style.setProperty('--pnf-hole-x', `${centerX}px`);
        frame.style.setProperty('--pnf-hole-y', `${centerY}px`);
        frame.style.setProperty('--pnf-hole-radius', `${radius}px`);
  ...
  return (
    ...
    >
      ...
      <div className='pnf-hole-placeholder' aria-hidden='true' />
      <canvas className='pnf-canvas' ref={canvasRef} aria-hidden='true' />
    </div>
  );
}
