 ...
 * parapets (tower tops), the wedge numerals (course and beat numbers).
 *
 * Every shape is a radial or stepped repeat: rhombus petals around a disc
 * for the Ishtar Gate rosette, three-step merlons for the Babylonian
 * crenellation, vertical wedges for the numerals. No figure appears. Colors
 * come from the sheet through class names so the geometry remaps with the
 * theme where it should and stays glazed where it should not.
 */
...
/* ------------------------------------------------------------------ *
 * The medallion: a lapis disc, sixteen gold petals, two gold rings. The
 * script sample is HTML laid over the center by the host.
 * ------------------------------------------------------------------ */
...
export default function Rosette({ size = 120, className }: RosetteProps) {
  ...
  return (
    ...
    >
      ...
      <circle className='gb-ros-ring' cx={c} cy={c} r={r1 - 3} />
    </svg>
  );
}
 ...
 * register rules. One pattern tile, repeated across any width.
 * ------------------------------------------------------------------ */
    ...
    >
      <defs>
        <pattern id={id} width={MERLON_W} height={PARAPET_H} patternUnits='userSpaceOnUse'>
          ...
          <rect className='gb-glaze' x={18} y={0} width={12} height={12} />
        </pattern>
      </defs>
