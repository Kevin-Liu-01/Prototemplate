 ...
 *
 * Home: the page's one dark moment, the lattice inverted. Everywhere else
 * the ornament is lapis glaze on fired clay; here the body of the wall is
 * lapis, the Ishtar Gate's true ground, with obsidian holes where the
 * dither leaves a brick unglazed, stepped ziggurat setbacks at both edges,
 * crenellated merlons over a gold course, and a gold foot. Four glazed
 * panels sit in it, one per product surface, each under its own header
 * course: the libraries as the shipped Next.js sample in a code window,
    ...
    <section className='bl-sec bl-kiln' id='platform'>
      ...
      <div className='bl-in'>
        <header className='bl-head bl-panel is-kiln'>
          <PatternSwatch field='parapet' size={40} brick={16} ground='none' className='is-mark' />
        ...
        <div className='bl-kiln-grid'>
          ...
          <article className='bl-panel is-kiln bl-kp is-lib'>
            <div className='bl-ch'>
              <h3>Libraries</h3>
              <code className='bl-ch-note'>6 first-party SDKs</code>
            </div>
            <div className='bl-kp-body'>
              <p className='bl-kp-lead'>
                The <code>&lt;T&gt;</code> component wraps JSX; <code>useGT()</code> and <code>getGT()</code>{' '}
                translate strings; <code>&lt;Num&gt;</code> and <code>&lt;DateTime&gt;</code> format for the
                locale.
              </p>
          ...
          <article className='bl-panel is-kiln bl-kp is-cli'>
            <div className='bl-ch'>
              <h3>CLI</h3>
              <code className='bl-ch-note'>npx gt translate</code>
            </div>
            <div className='bl-kp-body'>
              <p className='bl-kp-lead'>
                The CLI reads the project, sends only the new and changed strings, and writes every locale file
                into the build.
              </p>
          ...
          <article className='bl-panel is-kiln bl-kp is-dash'>
            <div className='bl-ch'>
              <h3>Dashboard</h3>
              <code className='bl-ch-note'>workspace · es-419</code>
            </div>
            <div className='bl-kp-body'>
              <p className='bl-kp-lead'>
                Every string beside its source. Approve it, edit it, or send it back. History keeps each
                revision.
              </p>
              <div className='bl-ws'>
                <div className='bl-win-bar'>
                ...
                <ul className='bl-ws-rows'>
                  {REVIEW_ROWS.map((row) => (
                    <li className='bl-ws-row' key={row.key}>
                      ...
                      <span className={`bl-stamp is-${row.state}`}>
                        {row.state === 'approved' ? 'approved' : 'edit'}
                      </span>
                    </li>
                  ))}
                </ul>
          ...
          <article className='bl-panel is-kiln bl-kp is-agent'>
            <div className='bl-ch'>
              ...
              <code className='bl-ch-note'>PR #218</code>
            </div>
            <div className='bl-kp-body'>
              <p className='bl-kp-lead'>
              ...
              <ol className='bl-trace'>
                ...
                <li>
                  ...
                  <span className='bl-trace-v'>
                    + <code>&lt;T&gt;</code> · + <code>&lt;DateTime&gt;</code> ·{' '}
                    <code>toLocaleDateString()</code> removed
                  </span>
                </li>
                <li>
