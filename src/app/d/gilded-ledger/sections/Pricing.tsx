...
/**
 * M17, Pricing, laid out as a bound ledger spread. A price is a number in a
 * column, so the module leads with the full published rate table as a real
 * ruled table: workflow in the label gutter, rate right-aligned, GT-library
 * variant in a third column. These are the only rates that may appear
 * anywhere. The two plans stand on the facing leaf as the accounts; a
 * doubled gold rule binds the two leaves (deco home C1.2). Nothing on this
 * module moves on its own.
 */
  ...
  return (
    <section className='tc-sec' id='pricing' ref={root}>
      ...
      <div className='gl-spread'>
        {/* The left leaf: the published rate ledger. Selected row treatment
            is reserved for the rate a reader is most likely pricing first,
            build time, and it is carried by weight alone. */}
        <div className='gl-spread-page is-ledger' data-reveal>
          <div className='df-rates'>
        ...
        {/* The right leaf: the two accounts, one above the other, divided by
            the page's hairline. The binding rule between the leaves is drawn
            once by this leaf's own edge. */}
        <div className='gl-spread-page is-accounts'>
      ...
      <div className='tc-compare' data-reveal>
        <a href='https://generaltranslation.com/pricing'>
