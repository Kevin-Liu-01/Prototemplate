...
export default function Frameworks() {
  ...
  useGSAP(
    () => {
      ...
      /* The underline must still be PLACED under reduced motion (it is the
         active-tab indicator), but it never travels and the panel never
         crossfades: the markup pose is the still. */
      ...
      if (settled.current && !reduce && panel.current) {
        gsap.fromTo(
          panel.current,
          { autoAlpha: 0, y: 6 },
  ...
  return (
    <section className='tc-sec' id='frameworks' ref={root}>
      ...
      >
        {/* Mounted, like every other code surface on the page: the engraved
            plate in its one gold ring, then the card, then the sample. */}
