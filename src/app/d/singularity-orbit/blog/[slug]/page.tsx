...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    title: post
      ? `${post.title} — Blog — Orbit — GT Redesign`
      : 'Blog — Orbit — GT Redesign',
 ...
 * to. The shell is the same one every Orbit company page runs; the
 ...
 */
export default async function SingularityOrbitBlogPostPage({ params }: Params) {
  const { slug } = await params;
  const post = findPost(slug);
  if (!post) notFound();
