...
export function isoCircle(
  r: number,
  z = 0
): { rx: number; ry: number; cy: number } {
  return {
    rx: r * ISO_COS30 * Math.SQRT2,
    ry: r * ISO_SIN30 * Math.SQRT2,
    cy: -z,
  };
}
...
/* ---------------------------------------------------------------------------
   Flat art in a surface — seating 2D drawings into a projected plane.
--------------------------------------------------------------------------- */
