...
/**
 * The page's only entrance: elements marked `data-reveal` rise a few pixels
 * and resolve. No scale, no blur, no stagger longer than a beat — the page
 * has to read the same in a screenshot.
 */
export function useQuietReveal(
  ...
  useGSAP(
    () => {
      ...
      ScrollTrigger.batch(
        gsap.utils.toArray<HTMLElement>('[data-reveal]', scope.current),
        {
          start: 'top 92%',
          once: true,
          onEnter: (batch) =>
            gsap.fromTo(
              ...
              /* The compositor hint lives only as long as the tween. A standing
               ...
               1px seam rules — the same paint-over engine.css already patches
               for .tc-hatch. */
