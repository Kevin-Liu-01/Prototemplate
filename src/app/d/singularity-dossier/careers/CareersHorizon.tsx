...
export default function CareersHorizon() {
  ...
  useGSAP(
    () => {
      ...
      const wordElements = Array.from(
        orbit.querySelectorAll<HTMLElement>('.careers-orbit-word')
      );
      let orbitRadius = 320;
      ...
      const words: OrbitWord[] = wordElements.map((wordRoot) => {
        ...
        const letters = parts.filter(
          (part) => !part.classList.contains('careers-orbit-flag')
        );
        const flagElement = parts.find((part) =>
          part.classList.contains('careers-orbit-flag')
        );
        return {
          root: wordRoot,
      ...
      const fit = () => {
        ...
        const orbitV = wide
          ...
          : Math.min(radius * 1.15, width * 0.8) * MOBILE_ORBIT_TILT + wordPad;
        const centerX = width / 2;
        const centerY = Math.max(
          Math.min(height * (wide ? 0.48 : 0.46), height - orbitV - edgeInset),
          orbitV + edgeInset
        );
        orbitTilt = wide ? WIDE_ORBIT_TILT : MOBILE_ORBIT_TILT;
        orbitRadius = wide
          ? radius * ORBIT_RADIUS_SCALE
          : Math.min(radius * 1.15, width * 0.8);
        packWords();
  ...
  return (
    ...
    >
      <canvas
        className='careers-horizon-canvas'
        ref={canvasRef}
        aria-hidden='true'
      />
      <div className='careers-orbit' ref={orbitRef} aria-hidden='true'>
        {LOCALE_ORBIT.map((entry) => (
          <span className='careers-orbit-word' key={entry.name}>
            <LocaleFlag
              locale={entry.locale}
              className='careers-orbit-flag'
              data-orbit-part
            />
            {entry.whole ? (
