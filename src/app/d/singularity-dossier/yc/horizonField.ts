/**
 * Renders an analytic event horizon into target 2D canvases through one
 * session-shared WebGL2 context. Output uses premultiplied alpha, pauses when
 * hidden or offscreen, and renders a static frame under reduced motion.
 */
...
export type HorizonParams = {
  ...
  radius: number;
  /** Host-space CSS-px position of this canvas's top-left corner. */
  ...
  ruleFadeOut: number;
  /** Concentric guide-ring radii, in rim units. */
...
export type HorizonOptions = {
   ...
   * prefers-reduced-motion; fields that omit it remain pointer-independent.
   */
...
/* These constraints stay outside the GLSL string so they do not ship in the
   bundle:
   - pal/toneMap use the shared prismatic palette and tanh tone map.
...
if (halo > 0.002) {
float hrad = 52.0 + min(uFxVel * 12.0, 18.0);
float inHalo = (1.0 - smoothstep(hrad * 0.55, hrad, cd)) * halo;
if (inHalo > 0.002) {
...
function getEngine(): Engine | null {
  ...
  engine = {
    ...
    draw(
      ...
      ctx.uniform2f(
        uWorldOrigin,
        params.worldOrigin[0] * dpr,
        params.worldOrigin[1] * dpr
      );
...
function sizeOf(sub: Subscriber): { width: number; height: number } {
  const width = Math.max(
    1,
    Math.floor((sub.target.clientWidth || 1) * sub.dpr)
  );
  const height = Math.max(
    1,
    Math.floor((sub.target.clientHeight || 1) * sub.dpr)
  );
  return { width, height };
}
...
function stepEffect(fx: EffectState, now: number) {
  ...
  const strengthK =
    1 - Math.exp(-dt * (fx.strengthTarget > fx.strength ? 7.5 : 4.5));
...
function attachEffects(
  ...
  const state: EffectState = {
    ...
    modes: options.modes,
    mode:
      initial === 'off' || options.modes.includes(initial) ? initial : 'off',
    hovering: false,
    engaged: false,
    pinned: false,
    x: 0.5,
  ...
  const aim = (event: PointerEvent): boolean => {
    ...
    state.velRaw = Math.min(
      Math.hypot(
        (nx - state.targetX) * rect.width,
        (ny - state.targetY) * rect.height
      ) /
        dt /
        1000,
      3
...
export function createHorizonField(
  ...
  return {
    ...
    destroy() {
      ...
      // The shared context remains alive for other subscribers in the session.
    },
  };
}
