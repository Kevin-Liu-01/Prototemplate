...
import { prefersReducedMotion } from './lang';

import './edge-globe.css';

/** The iso-drawing prop contract. */
type IsoProps = {
  className?: string;
  /** Base stroke weight in viewBox units; defaults to the stylesheet's value. */
  strokeWidth?: number;
...
/**
 * Edge delivery — the translation CDN as an orthographic globe: five
 * points of presence on a sparse graticule, one user, and one highlighted
 * great-circle route between the user and the PoP that answers. Depth is
 * carried by ink alone — near graticule arcs draw at the family's regular
 * weight, far arcs are dashed hairlines, and the limb is the strongest
 * neutral line; no fills, no shading.
 *
 * The meridian set is symmetric about the camera (±18°, ±54°) so no
 * meridian ever projects as a dead-vertical line. The five PoPs sit on
 * graticule intersections; the user does not. Callout leaders are
 * corner-routed (node → elbow → horizontal run) and fan outward on both
 * sides, so no two ever cross.
 *
 * At rest the serving pair — fra and the user — wear the drawing's quiet
 * ink, indistinguishable from the other callouts; the route arc alone
 * holds the accent. Each serving-pair callout ships an invisible accent
 * copy of its lockup and leader, and the timeline breathes the copy in
 * and out at the landing beat, so the whole lockup flashes to the accent
 * and settles back as one gesture.
 *
 * Motion is one loop: a request dot rides the route out, a ring lands at
 * fra, and the payload chip rides back. Each landing's flash — fra's as
 * the request dot arrives, the user's as the payload chip settles, echoed
 * by a ring at the origin dot — is scheduled on the same timeline at the
 * landing beats, never on a parallel clock. Under reduced motion the chip
 * rests mid-route, the dot, rings, and flash overlays are removed, and
 * the still frame rests entirely in the quiet ink.
 */
...
/* 179, not the frame's optical middle: the left rail's ticks end at 76
   and the right rail's start at 284, so a 170-centred sphere would sit
   2px off the left ticks and 22px off the right — 179 splits the air
...
/* ---------------- graticule ----------------
   Three parallels, four meridians, two polar caps the meridians
   terminate on (stopping short of the poles avoids a starburst at each
   pole; leaving lon 0 out avoids a dead-vertical plumb line). The
   spacing puts every PoP below exactly on an intersection. */
...
/* ---------------- the network ----------------
   Five PoPs, every one on a graticule intersection. `eu-central-1` is
   the one serving this reader. */

type Pop = {
  code: string;
  /** The compact rendering's name — the PoP's airport code; the ap-*
      region codes overrun the viewBox at the phone step's 12px. */
  short: string;
  lat: number;
  lon: number;
...
/* The left rail reads iad then fra — the home PoP labels on its own side
   of the sphere, well below iad at 47.5% of the frame's height, and with
   a real 18-unit horizontal arm like the right rail's (an elbow pinned
   outside the limb leaves only a stub that reads as one straight
   diagonal). The elbow sits just inside the limb (x 78.8 at that
   height), so the arm crosses the sphere's edge exactly once, and the
   derived path clears iad's leader and every node dot by 21+ units. The
   right rail keeps nrt/sin/syd top-to-bottom. */
...
/* The reader's row lives in the right ledger, between sin and syd —
   same leader grammar as the PoPs. */
const ORIGIN = {
  lat: -8,
  lon: 30,
  elbow: [272, 172],
...
/* ---------------- the route ----------------
   ...
   away from the sphere's centre — to stay legible as a route rather
   than another graticule line. Slerp in view space; both endpoints face
   the reader. */
...
const CHIP_REST: P3 = ROUTE_PTS[Math.round(ROUTE_PTS.length * 0.55)] ?? {
  x: CX,
  y: CY,
  z: 1,
};
...
export default function EdgeGlobe({
      ...
      tl.set(req, { t: 0 }, 0)
        ...
        .to(
          res,
          {
            ...
            duration: 1.15,
            ease: 'power1.inOut',
            onUpdate: () => place(chipEl, res.t),
          },
          1.2
        )
        .to(chipEl, { autoAlpha: 0, duration: 0.35 }, 2.5);

      /* Arrival flashes: the receiving end's lockup and
  ...
  return (
    <svg
      className={['eg', accent ? 'eg-accent-on' : 'eg-accent-off', className]
        .filter(Boolean)
        .join(' ')}
    ...
    >
      ...
      {CAPS.map((arc, i) => (
        <path
          key={`c${i}`}
          className={arc.front ? 'eg-hair' : 'eg-back'}
          d={arc.d}
        />
      ))}
      ...
      {POPS.map((pop) => {
        ...
        return (
          <g key={pop.code}>
            ...
            <text
              className='eg-label eg-label-compact'
              x={pop.textX}
              y={ey + 2.6}
              aria-hidden
            >
            ...
            {pop.home ? (
              ...
              <g className='eg-flash' ref={fraFlashRef} aria-hidden>
                ...
                />
                <text
                  className='eg-label eg-label-wide'
                  x={pop.textX}
                  y={ey + 2.6}
                >
                  {pop.code}
      ...
      />
      <circle
        className='eg-node-home'
        cx={fmt(homePt.x)}
        cy={fmt(homePt.y)}
        r={3.1}
      />
      <circle
        className='eg-origin-dot'
        cx={fmt(ORIGIN_PT.x)}
        cy={fmt(ORIGIN_PT.y)}
        r={3}
      />
      ...
      <g
        className='eg-chip'
        transform={`translate(${fmt(CHIP_REST.x)} ${fmt(CHIP_REST.y)})`}
        ref={chipRef}
      >
        <rect
          className='eg-chip-bg'
          x={-7.5}
          y={-7.5}
          width={15}
          height={15}
