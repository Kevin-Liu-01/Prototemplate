...
- Nine mood slides, one inside each section plus the Blue Marble in Brand, each placed after a dense slide so the image is a rest, and each separated from the next opener by at least one content slide so the two image kinds alternate. Eight are the photographs and artworks that served as openers between round five and round six, made two-tone with the deck's own screen; the ninth, the Bowen compass rose, replaced the Narita runway in round eight. A mood slide is the image alone with a small credit plate in the lower right; it has no title and no sentence.
...
- One small solid `--paper` plate (no blur, no shadow) anchored to the lower right of the content box holds the credit line and nothing else: 15px at line-height 1.45 in titanium, letter-spacing 0.01em, the `.cap` size and the same setting as the opener credit, on 10px by 14px padding. The plate's bottom edge is the content box bottom (771), 73px above the bottom rule (844). Round seven set it at 13px to match the viewer's counter; the counter is chrome, not slide content, and 13px is under the grammar's 15px floor.
