...
- Eight section openers, one at the start of each section. Six are two-tone dithers of the brand's own shader engines: four rendered from Prototemplate directions in round five and restored in round seven, and two recut in round ten, Prototemplate and Glyphfield from the liquid metal material rendered in Glyphfield and Status and plan from the Singularity direction's horizon ring. Two, Blog and content and Developer experience, are color renders of the gem smoke material made in round six. Every opener carries the section name, one sentence stating what the section covers, and a credit line naming the material.
...
Kevin's directive, round ten, on the Prototemplate and Glyphfield and Status and plan openers: "i meant change the title slides on slide 60 and 79, its just that the dither on the things in the back look weird and should just be on something better looking." The four two-tone openers that read well (the Brand arc, the Design system flow lines, the Website glyph rain, the Documentation wireframe sphere) each carry one large hard-edged form at sheet scale, and the dither sits on that form as shading or glow. The two replaced crops had the dither as the subject: a 7.3x blow-up of the prism whose faces read as a checker, and a band of mid-tone with no edge. Round ten's rule for a two-tone opener is therefore one large hard-edged form on a quiet ground, with the plate area solid ink or paper.

Every file is 1600 by 900. The dithered files are JPEGs with a light twin, at quality 85 from round five and at quality 95 for the two round ten pairs, which were saved through Pillow from the two-color PNGs the pipeline writes; `scripts/build-deck.mjs` recognizes a two-tone image (98 percent of pixels at the two extremes) and inlines it as a two-color PNG, so the JPEG only has to keep every cell on its side of the threshold, and both round ten pairs round-trip with no cell changed. The two color openers are saved at quality 92 and the same image serves both themes. A slide references the light file in `src` and the dark file in `data-dark`. `scripts/build-deck.mjs` inlines `opener-*` and `detail-*` files at their native size at JPEG quality 88 instead of resampling them to 1280 pixels wide; the `mood-*` files are full-bleed too and get the same treatment.
...
| Documentation | `42-opener-documentation` | `opener-documentation.jpg` | `opener-documentation-light.jpg` | Lens Gate, the wireframe sphere and rules, two-tone |
...
| Prototemplate and Glyphfield | `60-opener-prototemplate` | `opener-prototemplate.jpg` | `opener-prototemplate-light.jpg` | liquid metal (Paper Shaders) rendered in Glyphfield, the diamond form, two-tone |
| Status and plan | `79-opener-status` | `opener-status.jpg` | `opener-status-light.jpg` | Singularity, the horizon ring's left arc from the 404 route, two-tone |

Slide numbers are the round seven numbering (85 slides), which round eight kept; the later move of Nearest-page routing from the end of Website into Documentation shifted that section's start from 43 to 42, so the eight sections start at 1, 16, 31, 42, 47, 56, 60 and 79. Round eight moved three mood slides within their sections, so the mood slide numbers below differ from round seven.

### Shader pipeline

The six two-tone openers went through the same Pillow steps, four in round five and two in round ten. Round five's source renders are 1600 by 900 captures of Prototemplate directions (`/d/<slug>?chrome=0`). Round ten's are 3200 by 1800 (a 1600 by 900 viewport at device scale factor 2), one from the local dev server on port 3005 and one from Glyphfield's `/shader-preview` route, so their boxes are in device pixels, twice the CSS values. Crop boxes are left, top, right, bottom in source pixels; a box may reach past the source where the render's own ground is black, and Pillow pads it with black.
...
- Image: the liquid metal material in its diamond form, white chrome on a black ground: a rotated square about 620 pixels wide in the upper right of the sheet, with two contour bands and a rounded-rectangle highlight outlined in dark, on solid ink. It is made in Glyphfield, the section's own tool, and it is the only opener in liquid metal. Round ten put it in place of the Prism Light crop (see Rejected candidates).
- Source: Glyphfield (`/Users/kevinliu/repos/glyphfield`), route `/shader-preview?materialId=paper-liquid-metal&diagnostics=1&live=1` on the local dev server (port 3012), Paper Shaders' Liquid Metal family, rendered the way the two color openers were: Chromium (Playwright, `--use-gl=angle --use-angle=metal --ignore-gpu-blocklist`), 1600 by 900 viewport at device scale factor 2, `devicePixelsSupported` cleared on the `paperShaderMount` handle, `setMinPixelRatio(2)` and `setMaxPixelCount(3200 * 1800 + 1)`, buffer verified at 3200 by 1800. Uniforms passed through `setUniforms` (scratchpad `openers-60-79/glyphfield/hi/render.mjs`, variant `lm-gem-c5`): `u_colorBack #000000`, `u_colorTint #ffffff`, `u_softness 0.05`, `u_shiftRed 0`, `u_shiftBlue 0`, `u_contour 0.6`, `u_repetition 3`, `u_distortion 0.07`, `u_offsetX 0`, `u_offsetY 0`, `u_scale 0.5`, `u_shape 3` (the diamond; `u_shape 1` is the circle of the sphere runner-up). The frame is the second of the script's three samples, taken at about 4, 5.5 and 7 seconds of playback, saved as `hi/lm-gem-c5-s2.png` (3200 by 1800).
- Crop: -1325, 105, 2709, 2374 (4034 by 2269), computed from the object's bounding box in the source (pixels above 30, 1563 pixels across) so that the diamond is 620 pixels wide and centered at sheet x 1160, y 315. The box reaches past the source on the left and at the bottom, where Pillow pads black, the render's own ground. The diamond's lower-left facet ends right of and above the plate's top-right corner. On the sheet the plate spans 137, 463, 877, 771 (the title wraps to two lines, so it is taller than the one-line plates); the nearest lit cell is 83 pixels from that rectangle, measured on the render, and 110 pixels from the 500-top box the candidates were screened against, and no cell inside the plate or in the 30 pixel band around it is paper.
- Processing: black point 20, gamma 1.0. 8.3 percent of cells are paper. The upper facet edges, the two contour bands and the highlight's outline survive the screen as solid lines; the lower facets darken toward the ground, so the lower silhouette is carried by the bands. The dither sits only inside the object as shading, the way it sits inside the Lens Gate sphere on the Documentation opener. The light twin is a shaded diamond with black bands on paper.
- Not chosen (round ten, the same pipeline; files under scratchpad `openers-60-79`): the same material's circle (`60-chrome-sphere`, `u_shape 1`, 580 pixels at the same position) reads as a shaded ball with a crisp rim and is the fallback if the diamond is judged too sharp; the Singularity horizon sweep from the 404 render the Status and plan opener uses (`60-nf-sweep`, box 445, 400, 1200, 825, black 50, gamma 0.85) is the strongest single image in the pool, 120 pixels clear of the plate, but with the ring on Brand and on Status and plan it would make three ring openers in one deck, and it becomes the pick here if the ring leaves slide 79; a smoke ring line (`paper-smoke-ring-line`, 0.9 percent of cells lit) and a spiral dithered to a single clean line and read at projection as an empty sheet with a line drawing; a liquid metal form at a larger scale (`60-liquid-metal-b`) came within 14 pixels of the plate corner with 676 lit pixels in the band around the plate; Wide Rule's rings (`60-wr-rise`) put 19 dotted cells under the plate.
...
- Image: the Singularity direction's horizon ring seen from its side: a bright core with glow flanks running the full height of the sheet on the right, entering at the top between x 1180 and 1600 and leaving at the bottom between x 980 and 1290, so the curvature shows and the image reads as the ring rather than as a vertical band. The lower left is solid ink under the plate. It bookends the Brand opener, which carries the same engine's top arc, at the deck's other end. Round ten put it in place of the Aurora Paper band (see Rejected candidates).
- Source: `http://localhost:3005/d/singularity-dossier/this-route-is-missing?chrome=0`, the catch-all 404 route, which mounts the Singularity direction's NotFound horizon band (`src/app/d/singularity/sections/NotFound.tsx`) with no orbit labels, which is what kept the ring off this slide in round eight. Dark theme, Chromium (Playwright, `--use-gl=angle --use-angle=metal --ignore-gpu-blocklist`), 1600 by 900 viewport at device scale factor 2, first frame 4 seconds after load (scratchpad `openers-60-79/render2x/singularity-dossier_this-route-is-missing-dark-y0-1.png`, 3200 by 1800). The page's copy starts at device x 1232 and its content rail sits at x 432, so the box below holds only the ring and hairlines the black point clips.
- Crop: 445, 508, 1200, 933 in device pixels (CSS 222.5, 254, 600, 466.5), 755 by 425, scaled 1.06x to cover 800 by 450: the ring's left arc from the top edge to its leftmost point at the bottom edge, right of the content rail and left of the copy. The nearest lit cell is 84 pixels from the plate rectangle (137, 540, 877, 771); no cell inside it is paper, and no lit cell touches the sheet's left edge.
- Processing: black point 50, gamma 0.85. 12.1 percent of cells are paper. The light twin is a heavy black arc on paper with the same presence.
- Not chosen (round ten): the same arc shifted down (`79-nf-bow`, box 445, 560, 1200, 985) is nearly vertical and can read as a plain gradient band, the defect the aurora had; the liquid metal circle at 900 pixels (`79-chrome-ball`) is a strong form with a hard lower rim, but its body is a field of dithered mid-tone and its outlined hollow reads as an eye, so it is held as the fallback if the ring is not wanted here; god rays (`paper-god-rays`, a fan from one point in the upper right) is the cleanest hard geometry after the ring but lights 1.9 percent of cells and carries a stray lit cell near 400, 880 in both twins; Liquid Metal Noir ran its form to 4 pixels from the plate corner with 728 lit pixels in the band around the plate; Wide Rule's rise (`79-wr-rise`) put three cells under the plate.
...
- Credit: Material: Singularity, a Prototemplate direction
...
- Placement: after Why the redesign, the first dense slide of Brand, so the section runs opener, four content slides, the Blue Marble, three content slides, the Rosetta Stone, five content slides, opener. Round seven placed it after The mark, directly before the Design system opener, which made it a second opener. Kevin asked for the image ("also i like the blue marble"); round six had it as the Status and plan opener.
...
- Placement: after Nearest-page routing, which follows the docs overview, and before Markdown twins, so a content slide separates it from the Blog and content opener. Round seven placed it after Markdown twins, directly before the opener; round eight placed it directly after the docs overview, before the routing slide moved into the section from the end of Website.
...
- Round ten, from Kevin's directive on slides 60 and 79: the Prototemplate and Glyphfield opener became a two-tone dither of Paper Shaders' liquid metal in its diamond form, rendered in Glyphfield at 3200 by 1800, and the Status and plan opener became the Singularity horizon ring's left arc cut from the 404 route's label-free band at 3200 by 1800, both through the shader pipeline above; the Prism Light and Aurora Paper crops moved to Rejected candidates; the credit and the alt text on both slides changed; the two new pairs are saved at JPEG quality 95 from the two-color PNGs. No slide moved and the slide count did not change.
...
- Singularity ring bottom arc: a strong image, held back because it repeats the Brand engine. Round eight tested it again as the Status and plan fallback (boxes 608, 604, 992, 820 and 640, 620, 960, 800 on `singularity-dark-1` and `-2`, gamma 0.85 and 1.0): the wider boxes catch the locale labels under the ring, and in the tight box the arc's bright edge runs under the title plate, so the aurora retone was kept. Round ten went around both problems by rendering the catch-all 404 route (`/d/singularity-dossier/this-route-is-missing`), whose NotFound band carries the ring with no labels, at device scale factor 2, and cutting the ring's left arc so it clears the plate by 84 pixels; that crop is now the Status and plan opener.
- Aurora paper at black point 90 to 140 with gamma 1.6 (round eight): the ground clips, but the band thins to a dotted line and the flare is the only feature; gamma 0.8 with black 90 kept the band's body and was used until round ten.
- Prism Light body (the Prototemplate and Glyphfield opener from round five to round nine): `prism-light-dark-1.jpg`, box 370, 606, 588, 729 (218 by 123), the wireframe prism between the rotating source labels and the translated labels in the beam, scaled 7.3x, black point 10, gamma 0.9. Replaced in round ten. Kevin: "the dither on the things in the back look weird and should just be on something better looking". At 7.3x the prism's faces read as a checker of dithered glass and its edges as soft lines, so the dither was the subject rather than shading on a form.
- Aurora Paper band (the Status and plan opener from round five to round nine): `aurora-paper-dark-2.jpg`, box 900, 540, 1231, 726 (331 by 186), black point 90, gamma 0.8, the round eight retone. Replaced in round ten under the same directive: the band dithered to a grey smear with no edge anywhere on the sheet.
